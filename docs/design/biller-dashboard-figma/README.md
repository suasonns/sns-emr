...
## Phase 2 — Claims, Denials, Eligibility, Payment Posting, Reports (2026-08-24)

All 5 remaining nav items that previously rendered `ComingSoonPage` now have real pages,
backed by new tenant-scoped list/summary endpoints following the Phase-1
`resolve_billing_scope_tenant_id` + `require_automated_billing` pattern. No fabricated data
anywhere — every summary rate/average returns `null`/"—" when its denominator is 0, matching
the project's standing policy.

- **`ClaimsManagementPage.tsx`** (`GET /billing/claims`): 4 metric cards (Total/Submitted/
  Accepted/Denied) + a 4-stage lifecycle pipeline panel (Draft Batching/Submitted/Accepted/
  Paid, mirroring `01-claims-management.png`) + the claims registry table. "Export Claims" /
  "Batch Submission" mockup actions are not wired (no backend for either yet) — the page uses
  the shared `PageHeader` default inert actions instead of inventing new ones.
- **`DenialsAppealsPage.tsx`** (`GET /billing/denials`): 4 metric cards (Total Denials/Appeal
  Rate/Overturn Rate/Avg Resolution Days) + a real denial-reasons breakdown bar panel (grouped
  from `Denial.reason_description`) + a static "Appeal Guidance Tips" panel (general CMS
  timely-filing guidance text, not tenant data) + the denial/appeal registry table. `Denial
  .status` maps to the mockup's "Appeal Status" column via a fixed label dict (Not Appealed/In
  Review/Overturned/Upheld/Written Off).
- **`EligibilityVerificationPage.tsx`** (`GET /billing/eligibility-roster`): 4 metric cards
  (Active Census/Eligible/Pending Re-Verify/Inactive) + the patient coverage roster table +
  an "Upcoming Reverifications" side panel (real rows due within 7 days). The mockup's "Run
  Batch Verification" action is not wired — there is no automated 270/271 clearinghouse feed
  in this codebase.
- **`PaymentPostingPage.tsx`** (`GET /billing/remittances`): 4 metric cards (Payments MTD/ERA
  Received/Posted/Pending Manual Match) + the ERA/remittance registry table + a "Remittance by
  Payer" bar panel + an "Unmatched Payments" panel (real `Payment` rows with
  `match_status` in `UNMATCHED`/`MANUAL_REVIEW`). No "Match" action is wired (manual
  posting/reconciliation write-path does not exist yet — read-only registry only).
- **`ReportsPage.tsx`** — **deliberate deviation from `02-billing-reports.png`.** The mockup
  shows a report-generation/scheduling system with named "Generated By" users, download links,
  and enabled/disabled schedules — none of that exists in this codebase (no report-file
  generation, no export pipeline, no scheduler). Rather than fabricate fake generated-report
  rows or fake staff names, this page instead surfaces **live read-only snapshot cards** (one
  per domain: Billing Readiness, Claims, Denials & Appeals, Eligibility, Payment Posting) pulled
  from the same real endpoints as their dedicated pages, with an explicit banner stating that
  scheduled/downloadable report generation is not implemented yet. Revisit this page if/when a
  real report-generation backend is built.
