...
placement + authorization correction) — see
`RNICA_PHASE3_REMEDIATION_REGISTER.md` P3-009/P3-017/P3-018.
...
visit). Authorization was corrected 2026-09-23 ("SFV AUTHORIZATION
CORRECTION") from a shared RN-scope capability check (which incorrectly
authorized ADMINISTRATOR/DPCS and PA/physician-tier roles) to a dedicated
qualifying-nursing-credential roster: `RN`, `LVN`/`LPN`, `NP`, and
`CASE_MANAGER` only when the caller's underlying discipline also
qualifies (refined 2026-09-23, "FINAL AUTHORIZATION CORRECTION" —
"Case Manager" is a job title, not itself a credential). Rows below are
left as originally written for
audit-trail history; each affected row now has a **"Remediated:"**
follow-up note recording the current, verified state.
...
| SFV-P02 | Original LVN completes a different visit | Allow when authorized | Accept when authorized | Completion recorded | **GAP** — same as SFV-P01; "when authorized" cannot be evaluated because there is no authorization-service call in the current hook, only a discipline string check. *Remediated: `can_complete_sfv` now performs an explicit authorization check (tenant + patient access + qualifying-nursing-credential role check, corrected 2026-09-23 to a dedicated RN/LVN/NP roster, plus CASE_MANAGER conditioned on underlying nursing discipline, rather than a shared RN-scope capability); covered by the authorized-LVN test.* |
...
| SFV-P05 | Authorized on-call RN completes different visit | Allow | Accept | On-call RN and visit recorded | **NOT_VERIFIED** — no on-call/assignment authorization model was found gating this hook; "on-call" status is not read anywhere in `hope_phase_b_engine.py` or `_maybe_complete_open_sfv_for_visit`. *Remediated (by generic design, not a bespoke on-call feature): per product direction not to build a new on-call subsystem, `can_complete_sfv` grants access to any active, tenant/patient-authorized NURSING clinician holding a qualifying nursing credential (RN/LVN/NP, or CASE_MANAGER when the underlying discipline also qualifies) — an on-call RN who is such a user is automatically permitted with no on-call-specific code, since this repository has no distinct on-call/covering/per-diem role string. Still NOT_VERIFIED: whether the agency's real-world on-call assignment is reflected in who holds the RN role is outside this codebase's scope.* |
...
| SFV-P07 | Other authorized clinician completes different visit | Reflect configured permission | Accept only when permission and scope allow | Role and authority recorded | **GAP** — no permission-service integration exists in the current hook; only RN/LPN/LVN discipline strings are checked. *Remediated, then corrected (2026-09-23, "SFV AUTHORIZATION CORRECTION"), then refined (2026-09-23, "FINAL AUTHORIZATION CORRECTION"): the new endpoint calls `can_complete_sfv`, a real authorization check reused from `patient_access.py`. Initially this reused the shared RN-scope `PERFORM_RN_ASSESSMENT`/`FINALIZE_RN_DOCUMENTATION` capabilities, which are ALSO granted to ADMINISTRATOR/DPCS/DPCS_ADMINISTRATOR and to PA/physician-tier roles — verified by direct test to incorrectly authorize an Administrator and a Physician Assistant to complete an SFV. Corrected to a dedicated qualifying-nursing-credential roster (RN/LVN/NP), then refined so `CASE_MANAGER` ("Case Manager" is a job title, not a credential) authorizes only when the caller's underlying `User.discipline` also normalizes to a qualifying nursing credential — a Social Worker Case Manager or undocumented-discipline Case Manager is rejected; see `SNS_CONSTITUTION.md` §31 for the full authorized/excluded role lists. Still GAP beyond that roster — no broader configurable permission-service integration was built, matching the directive's no-new-subsystem instruction.* |
...
- **Remediated this pass (real authoritative command + frontend
  ...
  SFV-P05/P06/P07 (dedicated qualifying-nursing-credential roster, not a
  bespoke on-call feature and not the shared RN-scope capability
  originally used), SFV-P08 (frontend now blocked, not just backend),
  SFV-P19 (idempotent replay), SFV-P20 (verified with a real concurrency
  test through the endpoint), SFV-P25 (client-writable surface added but
  never trusts client identity), SFV-P27 (authoritative read endpoint now
  exists on both screens). See `backend/tests/test_sfv_completion_api.py`
  (23 tests, including PA/Administrator/Social-Worker/Chaplain/Volunteer/
  QA*/platform-role/non-nursing-Case-Manager rejection and
  NP/LVN/LPN/RN-Case-Manager/LVN-Case-Manager/on-call-RN/on-call-LVN
  authorized-success cases) and
...
- **Still an architectural gap, explicitly not built (per directive, to
  ...
  on the qualifying-nursing-credential roster, not a dedicated on-call
  feature).
