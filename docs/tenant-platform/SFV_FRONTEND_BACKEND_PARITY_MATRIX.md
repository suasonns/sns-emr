...
qualifying-nursing-credential roster: `RN`, `LVN`/`LPN`, `NP`,
`CASE_MANAGER` only. Rows below are left as originally written for
...
| SFV-P02 | Original LVN completes a different visit | Allow when authorized | Accept when authorized | Completion recorded | **GAP** — same as SFV-P01; "when authorized" cannot be evaluated because there is no authorization-service call in the current hook, only a discipline string check. *Remediated: `can_complete_sfv` now performs an explicit authorization check (tenant + patient access + qualifying-nursing-credential role check, corrected 2026-09-23 to a dedicated RN/LVN/NP/CASE_MANAGER roster rather than a shared RN-scope capability); covered by the authorized-LVN test.* |
...
- **Remediated this pass (real authoritative command + frontend
  ...
  (14 tests, including PA/Administrator/Social-Worker rejection and
  NP/LVN authorized-success cases) and
