...
| HOPE Item Set (data-element/technical specification, as distinct from the guidance manual's prose) | **VERIFIED BY CMS** (this pass, Issue #149) — retrieved directly from `https://www.cms.gov/files/document/hope-v1-02-all-item.pdf` (valid `%PDF-1.4`, 411,177 bytes, 14 pages, full text extracted and read). **Version-label discrepancy noted, not smoothed over**: the download URL is named `hope-v1-02-all-item.pdf`, but every page footer inside the document itself reads "v1.01 Effective October 1, 2025" — there is no internal "v1.02" marking anywhere in the item set body. This is consistent with the already-verified v1.01→v1.02 Change Table's own statement that v1.02 changed guidance-manual *prose* only (clarifications/corrections), not the item set's structure or codes — so the item set legitimately retaining its v1.01 footer, while CMS's website groups/names the download alongside the v1.02 release, is a plausible and non-contradictory explanation, but is recorded here as an open footnote rather than assumed. | n/a | Yes — full text (14/14 pages) | **VERIFIED BY CMS** (with the version-label footnote above) |
