...
|---|---|---|---|---|---|---|---|---|
| 1a | **[SUPERSEDED — see `J2052_J2053_LINEAGE_AUDIT.md`]** J2052 completion status/date is actively synced from the authoritative `SFVRequirement` on every RNICA-screen load (`SfvStatusCard`, `RNICA.jsx:7959-7979,8401-8404`) — not legacy/abandoned, but a fragile, pull-based, UI-only sync with no non-UI path | CRITICAL | RNICA/HOPE integration | None — `SFVRequirement` already authoritative | None | None required for current UI-driven export; document as a prerequisite condition for the future server-side HOPE Generation service | Future server-side HOPE Generation must read `SFVRequirement` directly, not the denormalized `form_data.sfv.*` copy | Release-blocking once export is automated/server-side (no non-UI sync path exists today) |
| 1b | **[SUPERSEDED — see `J2052_J2053_LINEAGE_AUDIT.md`]** J2053 (SFV symptom impact) has **no capture path on the completion visit at all** — `VisitNotes.jsx`'s `SymptomFollowUpVisitSection` sends only `{ completionVisitId }` (`VisitNotes.jsx:1062`); the only editable UI for `symptomImpactAtSfv.*` is on the RNICA triggering form itself (`RNICA.jsx:9518-9525`) | CRITICAL | RNICA/HOPE integration | Product/schema decision: where does symptom-impact-at-SFV live on the completion visit | Extend `SFVRequirement` or `Visit` with an impact-at-completion payload | Add symptom-impact fields to `SymptomFollowUpVisitSection`; remove/retire the RNICA-form `sfv` section's `symptomImpactAtSfv.*` fields once the correct path exists | Release-blocking — a clinician using the correct, authoritative completion workflow cannot document J2053 today |
...
| 8 | **[RECLASSIFIED CRITICAL per "RNICA \u2192 HOPE CRITICAL PATH REVIEW", 2026-09-23]** No `audit_event()` calls exist for any HOPE workflow transition (close/ready/export/submission-update/unlock/inactivation) or the SFV completion endpoint | CRITICAL | Compliance/audit | None | None (reuse existing `audit_events.py`) | None | Add `audit_event()` calls at each transition point | Compliance risk — no append-only trail exists for regulator-facing workflow actions today; hospice records require attributable, authenticated, auditable documentation |
...
| 19 | **[RECLASSIFIED CRITICAL per "RNICA \u2192 HOPE CRITICAL PATH REVIEW", 2026-09-23]** `RnicaHopeSubmissionAttempt` table exists but has zero writers anywhere in the codebase — a fully-built schema doing nothing | CRITICAL | HOPE submission | #4, #5 | None (schema complete) | None yet | Wire creation of a row per submission attempt once #4/#5 are addressed | Represents pre-built capacity that should be reused, not re-designed, when #4/#5 are implemented; dead submission schema blocks any real acceptance/rejection/correction tracking |
...
- **CRITICAL (7):** #1a, #1b, #2-#5, #8, #19 — reclassified per "RNICA
  → HOPE CRITICAL PATH REVIEW" (2026-09-23): these represent the
  core "RNICA is not complete until HOPE can complete its lifecycle"
  gap — the split J2052 (fragile sync)/J2053 (no capture path at all)
  finding, four consecutive missing pipeline stages (generation,
  validation, submission, acceptance-recording), the complete absence
  of an audit pipeline, and the dead submission-attempt schema. Audit
  (#8) and dead submission schema (#19) were reclassified from HIGH to
  CRITICAL this pass because a missing audit trail and an unusable
  submission-history table are both direct blockers to a compliant
  HOPE lifecycle, not enhancements to one.
- **HIGH (5):** #6, #7, #10, #11, #20 — correction workflow, medications
  coverage, and the CMS-rule content prerequisite.
...
1. Fix #1a/#1b (SFV field lineage — J2052 sync hardening + J2053
   completion-visit capture path) per `J2052_J2053_LINEAGE_AUDIT.md`
   immediately — smallest, most isolated, highest-severity, and does
   not require any new architecture beyond the SFV subsystem already in
   place.
