...
| Advance Directives / POA / Decision Maker / CPR Preference (approved target: 6 ACP fields total, not 3) | Target authority requires 6; current repository enforces 3 | **Current: not found in `RN_ICA_REQUIRED_FIELD_GROUPS` (3 of 6 target fields enforced).** Target 6th-field set and controlling HOPE mappings not yet verified. Target authority stands; repository state is discovery evidence only. | ACP & Goals of Care | Current: no Lock effect. Target: must block once implemented. `[IMPLEMENTATION DISCOVERY REQUIRED]` | n/a today |
