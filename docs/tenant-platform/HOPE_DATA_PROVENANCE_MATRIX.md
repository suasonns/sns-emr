...
| SFV | J2052, J2053 | **SFV-linked item within the applicable ADM/HUV record** | **SFV** — the exact linked completion visit identified by `SFVRequirement.completed_visit_id`, never a generic follow-up visit, the triggering assessment, or a patient-level "latest visit" | Backend: `SFVRequirement.completed_visit_id` → `ClinicalNote.content.symptom_impact`, `backend/app/api/visits.py` (`_extract_symptom_impact_from_content`, `list_sfv_requirements`); frontend: `hopeReportMapper.js` reads `sfvRequirement.status`/`.completedAt`/`.symptomImpact` exclusively. **RESOLVED** (was the single highest-priority provenance defect in this matrix; corrected by commits `b4da769` (J2052) and `0f177a3` (J2053) — see `J2052_LINEAGE_VERIFICATION.md`) | **Yes — single authoritative source (SFV), no RNICA self-attestation fallback remains** |
...
- **HUV1/HUV2/Discharge (DC) timepoint provenance is not yet independently mapped** for any section in this matrix — every row above traces only the ADM timepoint (plus the resolved SFV row, which is timepoint-agnostic by construction since it is keyed off `completed_visit_id`, not off which of ADM/HUV1/HUV2 triggered it). This is required before `HopeGenerationService` implementation may begin per `HOPE_GENERATION_SERVICE_DESIGN.md`.
