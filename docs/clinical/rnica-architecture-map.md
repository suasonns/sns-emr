...
**2026-09-08 (legacy writer removal verification)**

**Finding 1**: The legacy frontend deterministic narrative writer (`clinicalNarrativeBuilder.js` /
`buildClinicalNarrative()`) — previously only demoted to a fallback for unsaved assessments or
failed V2 calls (see Section 13 row above this one) — remained reachable in theory, which was
judged an unacceptable residual risk regardless of how rarely it would fire in practice.

**Impact 1**: Any code path capable of reproducing the banned report-style narrative
(HOSPICE CLINICAL PICTURE / REASON FOR HOSPICE ADMISSION / EVIDENCE OF DECLINE /
DISEASE-SPECIFIC SUPPORT) is a live risk to the single-source-of-truth guarantee, even if
unreachable in the current UI flow.

**Decision 1**: Delete the file and its test entirely (no rename/retention). Extract the shared,
engine-agnostic infrastructure it also held (Disease Trajectory constants/helpers, narrative
context fingerprint, Narrative Quality Gate) into a new, independently-tested module
(`narrativeSupport.js`) that is confirmed non-compositional (see Section 13 row for the full
export inventory and test results). Replace the unsaved-assessment fallback with a jargon-free
save-first UX backed by a new awaitable `saveNow()` action.

**Status 1**: `CODE_REMOVAL_VERIFIED` / `AUTOMATED_TESTS_VERIFIED` — file absence, zero remaining
executable references (grep across frontend `src/` and backend `app/`), and 276/276 frontend +
62/62 targeted backend tests all confirmed on commit `8fde005` (+ follow-up docstring cleanup
`f93988a`). `LIVE_PATIENT_CLICK_THROUGH_PENDING` — see Finding/Discovery 2 below.

**Finding 2**: When live in-browser re-verification was attempted against this removal, the
embedded browser tool returned `ERR_CONNECTION_REFUSED` / read timeouts. The first response
treated this as a browser-automation-tool problem and asked the user to choose between retrying,
skipping, or manual verification, without first checking whether the frontend/backend dev
processes themselves were still running.

**Impact 2**: `Get-NetTCPConnection` subsequently proved nothing was listening on port 5173 or
8000 at all — both dev servers had actually stopped (likely during the session-context-compaction
gap), which is a materially different condition from "the browser tool is flaky." Diagnosing it
as a tool issue first risked reporting a false verification blocker instead of the true root
cause, and would have wasted retries against servers that could never respond.

**Decision 2**: Before attributing a live-verification failure to browser-automation flakiness,
first check listening ports/process state and service health (`curl`/`Invoke-WebRequest` against
the health endpoint) from the correct worktree. Only treat the embedded browser tool itself as the
limiting factor once the underlying services are independently confirmed healthy via HTTP. In this
case: both servers were restarted from this exact worktree/branch/commit, confirmed listening
(frontend PID recorded via `Get-NetTCPConnection`, backend PID 47660) and returning HTTP 200
(`/` for frontend, `/health` for backend → `{"status":"ok"}`). The embedded browser tool completed
one login sequence successfully immediately after the restart, proving the services were reachable
end-to-end through the browser layer too, but then became unresponsive again across further
retries — that remaining unavailability is confirmed to be a tool-level limitation, not a service
outage, precisely because the services independently answer via direct HTTP the whole time.

**Alternatives considered**: Installing a new headless browser test framework (Playwright/
Cypress/Puppeteer) as a substitute end-to-end runner — rejected for this pass because none is
already a dependency of this repository (`sns-emr-frontend/package.json` has no such devDependency
and no `e2e`/`smoke` script), and introducing one under deployment time pressure was judged higher
risk than documenting the limitation honestly.

**Status 2**: Open — live patient click-through (Loren/Norma/Kessler) against commit `8fde005`/
`f93988a` remains unverified by direct browser observation. Reference:
`docs/planning/rnica_legacy_writer_removal_verification.md`.
