...
"""Targeted tests for the Cardiovascular Production Source Manifest v1
import (`import_cardiovascular_production_source_manifest.py`).
...
(`backend/manifests/cardiovascular_production_source_manifest_v1.json`),
...
`test_neurologic_production_source_manifest.py` (PR #37).
"""
...
def built_state():
    """Import the Cardiovascular Production Source Manifest v1 into a
    dedicated session against the test database, exactly once for this
    file (module-scoped -- see the identical rationale in
    test_neurologic_production_source_manifest.py: ontology tables are
    not tenant-scoped and are never cleared between tests)."""
...
def test_functional_status_dimension_not_present():
    """Approved vocabulary correction: FUNCTIONAL_STATUS is never a Tier 4
    variant dimension -- KPS/PPS/ADL are Tier 5 patient-state concepts."""
    dims = {v["dimension"] for d in MANIFEST["diseases"] for v in d.get("variants", [])}
    assert "FUNCTIONAL_STATUS" not in dims
    assert dims <= ALLOWED_VARIANT_DIMENSIONS


def test_kps_pps_adl_are_tier5_concepts_not_variants(built_state):
    diseases = built_state["diseases"]
    for disease_name, domain, name, _entry in _manifest_concepts():
        if name in ("KPS Less Than 70 Percent", "PPS Less Than 70 Percent", "ADL Dependence"):
            assert domain in ("HOSPICE_ELIGIBILITY_SUPPORT", "FUNCTIONAL_IMPACT")
    for disease_name, dimension, name in _manifest_variants():
        assert name not in ("KPS Less Than 70 Percent", "PPS Less Than 70 Percent", "ADL Dependence")
...
def test_congestive_heart_failure_remains_distinct_from_ischemic_heart_disease(built_state):
    diseases = built_state["diseases"]
    assert diseases["Congestive Heart Failure"].id != diseases["Ischemic Heart Disease"].id
...
# --- no changes to Neurologic ---

def test_no_neurologic_records_touched(db_session, built_state):
    from app.models.ontology_disease_blueprint import OntologyDiseaseVariant

    neurologic_disease_ids = {
        d.id for d in db_session.query(OntologyDisease).all()
        if d.disease_name not in ALL_DISEASE_NAMES
    }
    if not neurologic_disease_ids:
        pytest.skip("no non-Cardiovascular diseases present in this database")
    touched = (
        db_session.query(OntologyDiseaseVariant)
        .filter(OntologyDiseaseVariant.disease_id.in_(neurologic_disease_ids))
        .filter(OntologyDiseaseVariant.source_reference == "cardiovascular_production_source_manifest_v1")
        .count()
    )
    assert touched == 0
...
def test_only_authorized_files_changed():
    ...
    allowed_suffixes = (
        "backend/manifests/cardiovascular_production_source_manifest_v1.json",
        "backend/scripts/import_cardiovascular_production_source_manifest.py",
        "backend/tests/test_cardiovascular_production_source_manifest.py",
        "backend/artifacts/cardiovascular_production_manifest_acceptance_v1.json",
    )
