# tests/test_pulmonary_clinical_evidence_blueprint_v1.py
"""Targeted tests for the Pulmonary Clinical Evidence Blueprint v1 import
(`import_pulmonary_clinical_evidence_blueprint_v1.py`, PR #52).

Every assertion below is derived directly from the committed manifest file
(`backend/manifests/pulmonary_clinical_evidence_blueprint_v1.json`), never
...
manifests being importable first:
    ...
    - Pulmonary Production Source Manifest v1 (PR #39), which creates the
      "Chronic Obstructive Pulmonary Disease" AND "End Stage Pulmonary
      Disease" diseases this PR extends.
    - Functional Assessment Framework v1 (PR #49), which creates the
      "Functional Assessment Framework" disease this PR links to.

Unlike CHF (PR #50) and Dementia (PR #51), which each extend a single
disease, this manifest intentionally extends BOTH mirrored pulmonary
diseases identically -- PR #39 built them as two parallel disease
identities with an identical variant/concept structure, so extending
only one would create ontology drift between them.

This PR creates, PER extended disease:
    - 2 new atomic Pulmonary concepts (SYMPTOM x1 "Activity Intolerance",
      FINDING x1 "Severe Functional Limitation") -- both missing from PR
      #39's original concept set.
    - 2 new applicability edges linking those new concepts to PR #39's
      existing Pulmonary variants.
    - 2 OntologyRelationship edges linking PR #39's existing PPS Less
      Than 70 Percent / KPS Less Than 70 Percent concepts to PR #49's
      existing PPS / KPS scale-definition concepts.
    - 9 Pulmonary recertification-trend PROGNOSTIC_INDICATOR concepts.
...
It does NOT duplicate any PPS/KPS definition or any symptom/finding/
nutritional concept already present in PR #39. It does NOT make any
...
from scripts.import_pulmonary_production_source_manifest import (
    run as run_pulmonary_import,
    load_manifest as load_pulmonary_manifest,
)
from scripts.import_functional_assessment_framework_v1 import (
    run as run_faf_import,
    load_manifest as load_faf_manifest,
)
from scripts.import_pulmonary_clinical_evidence_blueprint_v1 import (
...
def built_state():
    ...
    then import the Pulmonary Clinical Evidence Blueprint extension
    ...
    try:
        run_pulmonary_import(session, manifest=load_pulmonary_manifest())
        session.commit()
        run_faf_import(session, manifest=load_faf_manifest())
        session.commit()

        counts = run_pulm_ext_import(session, manifest=MANIFEST)
        session.commit()
...
def test_manifest_extends_both_pulmonary_diseases():
...
def test_manifest_declares_two_new_concepts():
    assert len(NEW_CONCEPTS) == 2


def test_manifest_declares_two_new_applicability_edges():
    assert len(NEW_APPLICABILITY) == 2
...
def test_every_applicability_edge_declares_a_variant_dimension():
    """Pulmonary variant names are not unique across dimensions (e.g. 'End
    Stage Pulmonary Disease' exists as both a SEVERITY_CLASS and a
    DISEASE_PHASE variant) -- every applicability edge must disambiguate."""
...
def test_already_present_verified_mapping_covers_requested_items():
    ...
    assert "Hypoxemia" in requested
    assert "Cor Pulmonale" in requested
    assert "Unintentional Weight Loss" in requested
...
def test_extends_existing_pulmonary_disease_never_creates_a_new_one(built_state, disease_name):
...
def test_import_aborts_if_pulmonary_disease_is_missing():
    ...
    second Pulmonary foundation."""
    from scripts.import_pulmonary_clinical_evidence_blueprint_v1 import _resolve_disease
    session = TestSessionLocal()
    try:
...
def test_no_new_disease_variant_created_by_this_manifest(built_state):
    ...
    references PR #39's existing Pulmonary variants by name+dimension."""
...
def test_no_cartesian_applicability_generation(built_state):
    ...
    new concepts -- never one row per (new concept x every Pulmonary
    variant)."""
    session = built_state["session"]
    for disease in built_state["diseases"].values():
        count = 0
...
def test_every_faf_linkage_relationship_exists(built_state, entry, disease_name):
    session = built_state["session"]
    disease = built_state["diseases"][disease_name]
    # Pulmonary-side concepts here are HOSPICE_ELIGIBILITY_SUPPORT rows
    ...
    from scripts.import_pulmonary_clinical_evidence_blueprint_v1 import _resolve_concept
    pulm_concept = _resolve_concept(session, disease.id, entry["chf_concept_domain"], entry["chf_concept_name"])
...
def test_faf_linkage_never_duplicates_scale_definition(built_state):
    ...
    though it is referenced twice (once per extended pulmonary disease)."""
    session = built_state["session"]
...
def test_recertification_indicators_cover_pps_kps_weight_oxygen_hospitalization_respiratory_failure_symptom_narrative_ed_visit(built_state):
    names = {t["name"] for t in TREND_INDICATORS}
    expected = {
        "PPS Trend", "KPS Trend", "Weight Trend", "Oxygen Requirement Trend",
        "Hospitalization Trend", "Respiratory Failure Trend", "Symptom Burden Trend",
        "Narrative Trend", "ED Visit Trend",
    }
...
def test_second_import_creates_zero_new_rows():
    ...
    try:
        ...
        run_faf_import(session, manifest=load_faf_manifest())
        session.commit()
        run_pulm_ext_import(session, manifest=MANIFEST)
        session.commit()

        result = run_pulm_ext_import(session, manifest=MANIFEST)
        session.commit()
