...
"""Targeted tests for the Renal Production Source Manifest v1
import (`import_renal_production_source_manifest.py`).
...
(`backend/manifests/renal_production_source_manifest_v1.json`),
...
test_cardiovascular_production_source_manifest.py (PR #38), and
test_pulmonary_production_source_manifest.py (PR #39).
"""
...
def built_state():
    """Import the Renal Production Source Manifest v1 into a
...
def test_every_applicability_entry_declares_an_explicit_variant_dimension():
    """Critical LCD rule: no name-only variant matching is permitted --
    every applicability entry must carry an explicit variant_dimension,
    since e.g. 'Recurrent Hospitalization' exists as both a
    RECURRENCE_STATE variant name and a COMPLICATION concept name, and
    'End Stage Renal Disease'/'End Stage Renal Failure' recur across
    SEVERITY_CLASS and DISEASE_PHASE."""
...
def test_kps_pps_adl_are_tier5_concepts_not_variants():
    """Same vocabulary-correction pattern proven in the Cardiovascular
    and Pulmonary manifests -- KPS/PPS/ADL are Tier 5 patient-state
    concepts, never Tier 4 variants."""
...
def test_hospice_eligibility_support_concepts_require_evidence(db_session, built_state):
    """Critical LCD rule: renal diagnosis alone is insufficient --
    ...
    for disease_name, domain, name, _entry in _manifest_concepts():
        if domain != "HOSPICE_ELIGIBILITY_SUPPORT":
            continue
        concept = _concept(db_session, diseases[disease_name], domain, name)
        rule = db_session.query(OntologyEvidenceRule).filter_by(concept_type=domain, concept_id=concept.id).one_or_none()
...
def test_chronic_renal_failure_remains_distinct_from_acute_renal_failure(built_state):
    diseases = built_state["diseases"]
    assert diseases["Chronic Renal Failure"].id != diseases["Acute Renal Failure"].id
...
def test_only_authorized_files_changed():
    ...
    allowed_suffixes = (
        "backend/manifests/renal_production_source_manifest_v1.json",
        "backend/scripts/import_renal_production_source_manifest.py",
        "backend/tests/test_renal_production_source_manifest.py",
        "backend/artifacts/renal_production_manifest_acceptance_v1.json",
    )
