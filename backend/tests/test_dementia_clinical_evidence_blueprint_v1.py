# tests/test_dementia_clinical_evidence_blueprint_v1.py
"""Targeted tests for the Dementia Clinical Evidence Blueprint v1 import
(`import_dementia_clinical_evidence_blueprint_v1.py`, PR #51).

Every assertion below is derived directly from the committed manifest file
(`backend/manifests/dementia_clinical_evidence_blueprint_v1.json`), never
...
manifests being importable first:
    ...
    - Dementia Production Hardening v1 (PR #44), which creates the
      "Dementia Due To Alzheimer's Disease" disease this PR extends.
    - Functional Assessment Framework v1 (PR #49), which creates the
      "Functional Assessment Framework" disease this PR links to.

This PR creates:
    - 7 new atomic Dementia concepts across FINDING (1), FUNCTIONAL_IMPACT
      (3), NUTRITIONAL_IMPACT (2), and COMPLICATION (1) domains -- every
      one missing from PR #44's original Dementia concept set. Every
      other requested concept (7 major complications + "Loss Of
      Meaningful Verbal Communication") was verified already present in
      PR #44 under a clinically-equivalent name and is documented, never
      duplicated, in already_present_verified.mapping.
    - 7 new applicability edges linking those new concepts to PR #44's
      existing Dementia variants.
    - 3 OntologyRelationship edges linking PR #44's existing FAST Stage 7
      / PPS Less Than 70 Percent / KPS Less Than 70 Percent concepts to
      PR #49's existing FAST / PPS / KPS scale-definition concepts.
    - 10 Dementia recertification-trend PROGNOSTIC_INDICATOR concepts.
...
(no new FAST stages). It does NOT duplicate any FAST/PPS/KPS definition or
any major complication already present in PR #44. It does NOT make any
schema, migration, or API change.
"""
...
from scripts.import_dementia_production_hardening import (
    run as run_dementia_import,
    load_manifest as load_dementia_manifest,
)
from scripts.import_functional_assessment_framework_v1 import (
    run as run_faf_import,
    load_manifest as load_faf_manifest,
)
from scripts.import_dementia_clinical_evidence_blueprint_v1 import (
    ...
    run as run_dementia_ext_import,
)
...
def built_state():
    ...
    then import the Dementia Clinical Evidence Blueprint extension
    manifest exactly once for this file (module-scoped -- ontology
    tables are not tenant-scoped and are never cleared between tests by
    the function-scoped db_session fixture)."""
    session = TestSessionLocal()
    try:
        run_dementia_import(session, manifest=load_dementia_manifest())
        session.commit()
        run_faf_import(session, manifest=load_faf_manifest())
        session.commit()

        counts = run_dementia_ext_import(session, manifest=MANIFEST)
        session.commit()

        dementia_disease = session.query(OntologyDisease).filter_by(disease_name=EXTENDS_DISEASE_NAME).one()
        faf_disease = session.query(OntologyDisease).filter_by(disease_name=FAF_DISEASE_NAME).one()
        yield {"dementia_disease": dementia_disease, "faf_disease": faf_disease, "counts": counts, "session": session}
    finally:
        session.close()
...
def test_manifest_declares_extension_only_rules():
    ...
    assert rules["no_new_fast_stages"] is True
    assert rules["no_new_pps_kps_definitions"] is True
    assert rules["no_duplicate_assessment_definitions"] is True


def test_manifest_declares_seven_new_concepts():
    assert len(NEW_CONCEPTS) == 7
...
def test_already_present_verified_mapping_covers_eight_items():
    """All 7 requested major complications plus "Loss Of Meaningful
    Verbal Communication" were verified already present in PR #44 under
    a clinically-equivalent name -- none is duplicated by this manifest."""
    mapping = MANIFEST["already_present_verified"]["mapping"]
    assert len(mapping) == 8
    requested = {m["requested"] for m in mapping}
    assert "Loss Of Meaningful Verbal Communication" in requested
    assert "Aspiration Pneumonia" in requested
    assert "Albumin Less Than 2.5" in requested
...
def test_extends_existing_dementia_disease_never_creates_a_new_one(built_state):
...
def test_import_aborts_if_dementia_disease_is_missing():
    ...
    second Dementia foundation."""
    from scripts.import_dementia_clinical_evidence_blueprint_v1 import _resolve_disease
    session = TestSessionLocal()
    try:
...
def test_no_new_disease_variant_created_by_this_manifest(built_state):
    ...
    references PR #44's existing Dementia variants by name."""
...
def test_every_declared_new_concept_exists(built_state, entry):
    row = _concept(built_state["session"], built_state["dementia_disease"].id, entry["domain"], entry["name"])
...
def test_stored_new_concept_count_matches_manifest_exactly(built_state):
    session = built_state["session"]
    dementia_id = built_state["dementia_disease"].id
    total = 0
    for entry in NEW_CONCEPTS:
        row = _concept(session, dementia_id, entry["domain"], entry["name"])
        assert row is not None
        total += 1
...
def test_every_declared_applicability_edge_exists(built_state, entry):
    session = built_state["session"]
    dementia_id = built_state["dementia_disease"].id
    variant = _variant(session, dementia_id, entry["variant"])
    concept = _concept(session, dementia_id, entry["concept_domain"], entry["concept"])
...
def test_no_cartesian_applicability_generation(built_state):
    """Exactly the 7 declared applicability edges exist for the new
    concepts -- never one row per (new concept x every Dementia variant)."""
    session = built_state["session"]
    dementia_id = built_state["dementia_disease"].id
    count = 0
    for entry in NEW_CONCEPTS:
        concept = _concept(session, dementia_id, entry["domain"], entry["name"])
        count += (
            session.query(OntologyConceptVariantApplicability)
...
def test_every_faf_linkage_relationship_exists(built_state, entry):
    session = built_state["session"]
    # Dementia-side concepts here are FINDING / HOSPICE_ELIGIBILITY_SUPPORT
    # rows owned by PR #44, not by this manifest's CONCEPT_DOMAIN_MODEL_MAP,
    # so use the importer's full lookup helper (which covers both domains).
    from scripts.import_dementia_clinical_evidence_blueprint_v1 import _resolve_concept
    dementia_concept = _resolve_concept(session, built_state["dementia_disease"].id, entry["chf_concept_domain"], entry["chf_concept_name"])
...
def test_every_trend_indicator_exists(built_state, entry):
    ...
    row = (
        ...
        .filter_by(disease_id=built_state["dementia_disease"].id)
...
def test_recertification_indicators_cover_fast_pps_kps_speech_ambulation_transfer_weight_nutrition_complication_narrative(built_state):
    names = {t["name"] for t in TREND_INDICATORS}
    expected = {
        "FAST Trend", "PPS Trend", "KPS Trend", "Speech Trend", "Ambulation Trend",
        "Transfer Trend", "Weight Trend", "Nutrition Trend", "Complication History Trend",
        "Narrative Trend",
    }
...
def test_ai_layer_declares_allowed_capabilities():
    ...
    assert ai_may == {
        ...
        "fast_progression_detection",
        "nutritional_decline_detection",
        "increasing_dependence_detection",
...
def test_second_import_creates_zero_new_rows():
    ...
    try:
        ...
        run_faf_import(session, manifest=load_faf_manifest())
        session.commit()
        run_dementia_ext_import(session, manifest=MANIFEST)
        session.commit()

        result = run_dementia_ext_import(session, manifest=MANIFEST)
        session.commit()
...
def test_acceptance_report_shows_full_coverage(built_state):
    ...
    assert report["applicability"]["expected"] == report["applicability"]["stored"] == 7
    ...
    assert report["recertification_trend_indicators"]["expected"] == \
        report["recertification_trend_indicators"]["stored"] == 10
    assert report["recertification_trend_indicators"]["missing"] == []
    assert report["provenance_coverage"]["checked"] == report["provenance_coverage"]["valid"] == 17
