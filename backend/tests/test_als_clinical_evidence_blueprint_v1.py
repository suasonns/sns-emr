# tests/test_als_clinical_evidence_blueprint_v1.py
"""Targeted tests for the ALS Clinical Evidence Blueprint v1 import
(`import_als_clinical_evidence_blueprint_v1.py`, PR #55).

Every assertion below is derived directly from the committed manifest file
(`backend/manifests/als_clinical_evidence_blueprint_v1.json`), never from
...
manifests being importable first:
    ...
    - ALS Production Source Manifest v1 (PR #43), which creates the
      single "Amyotrophic Lateral Sclerosis" disease this PR extends.
    - Functional Assessment Framework v1 (PR #49), which creates the
      "Functional Assessment Framework" disease this PR links to.

This PR creates:
    - 7 new atomic ALS concepts across COMPLICATION (5) and
      HOSPICE_ELIGIBILITY_SUPPORT (2) domains -- every one missing from
      PR #43's original ALS concept set.
    - 7 new applicability edges linking those new concepts to PR #43's
      existing "Advanced Amyotrophic Lateral Sclerosis" variant.
    - 2 OntologyRelationship edges linking PR #43's existing PPS Less
      Than 70 Percent / KPS Less Than 70 Percent concepts to PR #49's
      existing PPS / KPS scale-definition concepts.
    - 9 ALS recertification-trend PROGNOSTIC_INDICATOR concepts.
...
It does NOT duplicate any PPS/KPS/ADL definition. It does NOT make any
...
from scripts.import_als_production_source_manifest import (
    run as run_als_foundation_import,
    load_manifest as load_als_foundation_manifest,
)
from scripts.import_functional_assessment_framework_v1 import (
    run as run_faf_import,
    load_manifest as load_faf_manifest,
)
from scripts.import_als_clinical_evidence_blueprint_v1 import (
...
def built_state():
    ...
    then import the ALS Clinical Evidence Blueprint extension manifest
    ...
    try:
        run_als_foundation_import(session, manifest=load_als_foundation_manifest())
        session.commit()
        run_faf_import(session, manifest=load_faf_manifest())
        session.commit()

        counts = run_als_ceb_import(session, manifest=MANIFEST)
        session.commit()

        als_disease = session.query(OntologyDisease).filter_by(disease_name=EXTENDS_DISEASE_NAME).one()
        faf_disease = session.query(OntologyDisease).filter_by(disease_name=FAF_DISEASE_NAME).one()
        yield {"als_disease": als_disease, "faf_disease": faf_disease, "counts": counts, "session": session}
    finally:
        session.close()
...
def test_already_present_verified_mapping_covers_requested_items():
    ...
    expected = {
        "Critically Impaired Breathing Capacity", "Vital Capacity Decline", "Respiratory Insufficiency",
        "Dependence On Ventilatory Support", "Rapid Disease Progression", "Dysphagia", "Progressive Weakness",
        "Severe Nutritional Impairment", "Significant Weight Loss", "PPS Less Than 70 Percent",
        "KPS Less Than 70 Percent", "ADL Dependence", "Progressive Functional Decline",
...
def test_extends_existing_als_disease_never_creates_a_new_one(built_state):
...
def test_import_aborts_if_als_disease_is_missing():
    ...
    second ALS foundation."""
    from scripts.import_als_clinical_evidence_blueprint_v1 import _resolve_disease
    session = TestSessionLocal()
    try:
...
def test_no_new_disease_variant_created_by_this_manifest(built_state):
    ...
    references PR #43's existing ALS variant by name."""
...
def test_every_declared_new_concept_exists(built_state, entry):
    row = _concept(built_state["session"], built_state["als_disease"].id, entry["domain"], entry["name"])
...
def test_stored_new_concept_count_matches_manifest_exactly(built_state):
    session = built_state["session"]
    als_id = built_state["als_disease"].id
    total = 0
    for entry in NEW_CONCEPTS:
        row = _concept(session, als_id, entry["domain"], entry["name"])
        assert row is not None
        total += 1
    assert total == len(NEW_CONCEPTS)


def test_new_complication_concepts_stored_in_complication_table(built_state):
    session = built_state["session"]
    als_id = built_state["als_disease"].id
    complication_names = {c["name"].strip().lower() for c in NEW_CONCEPTS if c["domain"] == "COMPLICATION"}
    stored = {
        row.complication_name.strip().lower()
        for row in session.query(OntologyDiseaseComplication).filter_by(disease_id=als_id).all()
    }
    assert complication_names.issubset(stored)
...
def test_new_hospice_eligibility_support_concepts_stored(built_state):
    ...
    stored = {
        ...
        for row in session.query(OntologyDiseaseHospiceEligibilitySupport).filter_by(disease_id=als_id).all()
    }
...
def test_every_declared_applicability_edge_exists(built_state, entry):
    session = built_state["session"]
    als_id = built_state["als_disease"].id
    variant = _variant(session, als_id, entry["variant"])
    concept = _concept(session, als_id, entry["concept_domain"], entry["concept"])
...
def test_no_cartesian_applicability_generation(built_state):
    ...
    concepts -- never one row per (new concept x every ALS variant)."""
    session = built_state["session"]
    als_id = built_state["als_disease"].id
    count = 0
    for entry in NEW_CONCEPTS:
        concept = _concept(session, als_id, entry["domain"], entry["name"])
        count += (
            session.query(OntologyConceptVariantApplicability)
...
def test_every_faf_linkage_relationship_exists(built_state, entry):
    session = built_state["session"]
    # ALS-side concepts here are HOSPICE_ELIGIBILITY_SUPPORT rows owned by
    # PR #43, so use the importer's full lookup helper.
    from scripts.import_als_clinical_evidence_blueprint_v1 import _resolve_concept
    als_concept = _resolve_concept(session, built_state["als_disease"].id, entry["chf_concept_domain"], entry["chf_concept_name"])
    ...
    relationship = (
        ...
        .filter_by(
            ...
            source_concept_id=als_concept.id,
...
def test_every_trend_indicator_exists(built_state, entry):
    ...
    row = (
        ...
        .filter_by(disease_id=built_state["als_disease"].id)
...
def test_recertification_indicators_cover_pps_kps_vc_weight_speech_swallow_resp_complication_narrative(built_state):
    names = {t["name"] for t in TREND_INDICATORS}
    expected = {
        "PPS Trend", "KPS Trend", "Vital Capacity Trend", "Weight Trend", "Speech Trend",
        "Swallowing Trend", "Respiratory Support Trend", "Complication History Trend", "Narrative Trend",
    }
...
def test_second_import_creates_zero_new_rows():
    ...
    try:
        ...
        run_faf_import(session, manifest=load_faf_manifest())
        session.commit()
        run_als_ceb_import(session, manifest=MANIFEST)
        session.commit()

        result = run_als_ceb_import(session, manifest=MANIFEST)
        session.commit()
...
def test_acceptance_report_shows_full_coverage(built_state):
    ...
    assert report["recertification_trend_indicators"]["expected"] == \
        report["recertification_trend_indicators"]["stored"] == 9
    assert report["recertification_trend_indicators"]["missing"] == []
    assert report["provenance_coverage"]["checked"] == report["provenance_coverage"]["valid"] == 16
