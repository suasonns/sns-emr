...
def test_non_disease_specific_support_cannot_establish_eligibility_alone(db_session, built_state):
    ...
    disease_specific_present = any(
        _find_concept_entry(ALS, "HOSPICE_ELIGIBILITY_SUPPORT", n)["source_classification"] == "LCD_DISEASE_SPECIFIC"
...
def test_only_authorized_files_changed():
    ...
    allowed_suffixes = (
        "backend/manifests/als_production_source_manifest_v1.json",
        "backend/scripts/import_als_production_source_manifest.py",
        "backend/tests/test_als_production_source_manifest.py",
        "backend/artifacts/als_production_manifest_acceptance_v1.json",
    )
