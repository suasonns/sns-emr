...
def save_facesheet(
    ...
    if "code_status" in data:
        code_status_value = data.get("code_status")

        if code_status_value and str(code_status_value).strip():
            set_current_code_status(
                db,
                patient_id=patient.id,
                tenant_id=tenant_id,
                code_status=str(code_status_value),
                source="FACESHEET",
                updated_by=user_id,
            )

        # patient_code_statuses is authoritative; the legacy facesheet
        # column is mirrored by the sync service itself, so drop it here
        # to avoid setattr() double-writing/overwriting the mirrored label.
        data.pop("code_status")

    physician_role_fields = {
        ATTENDING: {
            "name": "attending_physician_name",
            "address": "attending_physician_address",
            "phone": "attending_physician_phone",
            "fax": "attending_physician_fax",
            "npi": "attending_physician_npi",
            "will_follow_in_hospice": "attending_physician_following",
        },
        MEDICAL_DIRECTOR: {
            "name": "medical_director_name",
            "address": "medical_director_address",
            "phone": "medical_director_phone",
            "fax": "medical_director_fax",
            "npi": "medical_director_npi",
        },
        ASSOCIATE_MEDICAL_DIRECTOR: {
            "name": "associate_medical_director_name",
            "npi": "associate_medical_director_npi",
        },
    }

    for role, field_map in physician_role_fields.items():
        if not any(field in data for field in field_map.values()):
            continue
        ...
        set_physician_assignment(
            ...
            role=role,
            source="FACESHEET",
            name=data.get(field_map.get("name")),
            address=data.get(field_map.get("address")),
            phone=data.get(field_map.get("phone")),
            fax=data.get(field_map.get("fax")),
            npi=data.get(field_map.get("npi")),
            will_follow_in_hospice=data.get(field_map.get("will_follow_in_hospice")),
            updated_by=user_id,
        )

        # patient_physician_assignments is authoritative; the legacy
        # facesheet columns are mirrored by the sync service itself, so
        # drop them here to avoid setattr() double-writing/overwriting
        # the mirrored values.
    ...
    contact_role_fields = {
        RESPONSIBLE_PARTY: {
            ...
            "relationship_to_patient": "responsible_party_relationship",
            "phone": "responsible_party_phone",
        },
        EMERGENCY_CONTACT: {
            "name": "emergency_contact_name",
            "relationship_to_patient": "emergency_contact_relationship",
            "phone": "emergency_contact_phone",
        },
    }

    for role, field_map in contact_role_fields.items():
        if not any(field in data for field in field_map.values()):
            continue
        ...
        set_patient_contact(
            ...
            source="FACESHEET",
            name=data.get(field_map.get("name")),
            relationship_to_patient=data.get(field_map.get("relationship_to_patient")),
            phone=data.get(field_map.get("phone")),
            updated_by=user_id,
        )

        # patient_contacts is authoritative; the legacy facesheet columns
        # are mirrored by the sync service itself, so drop them here to
        # avoid setattr() double-writing/overwriting the mirrored values.
        for field in field_map.values():
            data.pop(field, None)
