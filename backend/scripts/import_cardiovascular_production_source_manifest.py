# scripts/import_cardiovascular_production_source_manifest.py
"""
Cardiovascular Production Source Manifest v1 -- Verbatim Importer.

Reads backend/manifests/cardiovascular_production_source_manifest_v1.json
(the sole authoritative source for this import -- never inferred,
reconstructed, or clinically re-derived) and creates the Cardiovascular
...
Run with: .\\.venv\\Scripts\\python.exe scripts\\import_cardiovascular_production_source_manifest.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "cardiovascular_production_source_manifest_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "cardiovascular_production_manifest_acceptance_v1.json"
)
IMPORTER_NAME = "import_cardiovascular_production_source_manifest"


SYSTEM_NAME = "Cardiovascular System"

# --- Tier 4 variant dimensions permitted by the
# ck_ontology_disease_variant_dimension CHECK constraint. Identical to the
# Neurologic importer's list -- this manifest uses only a subset of it
# (SEVERITY_CLASS, TREATMENT_STATE, DISEASE_PHASE, RECURRENCE_STATE,
# CARDIAC_CHAMBER); FUNCTIONAL_STATUS was explicitly excluded (approved
# vocabulary correction: KPS/PPS/ADL are Tier 5 patient-state concepts,
# never a Tier 4 variant dimension). ---
...
def _resolve_or_create_diseases(db: Session, manifest: dict) -> Dict[str, OntologyDisease]:
    ...
    Cardiovascular System body system, the declared family, and the
    disease itself if not already present. Never creates a new body
    ...
    if len(family_names) != 1:
        raise RuntimeError(
            f"Cardiovascular Production Manifest v1 declares {len(family_names)} families; "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id) -> bool:
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="cardiovascular_production_source_manifest_v1",
            evidence_type="MANIFEST_ATOMIC_CONCEPT",
            confidence="HIGH",
            patient_fact_requires_evidence=True,
            notes="Imported verbatim from the approved Cardiovascular Production Source Manifest v1.",
        )
    )
    return True
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Cardiovascular Production Manifest v1 failed structural/vocabulary validation: {errors}")
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for v in disease_entry.get("variants", []):
            ...
            variant = OntologyDiseaseVariant(
                ...
                source_reference="cardiovascular_production_source_manifest_v1",
            )
            db.add(variant)
            db.flush()
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for a in disease_entry.get("applicability", []):
            ...
            if variant_key_found is None or concept_key_found is None:
                raise RuntimeError(
                    f"Cardiovascular Production Manifest v1 applicability mapping references a variant/concept "
            ...
            edge = OntologyConceptVariantApplicability(
                ...
                variant_id=variant_key_found.id,
                applicability_type=applicability_type,
                description="Imported verbatim from the approved Cardiovascular Production Source Manifest v1.",
                evidence_requirement=(
