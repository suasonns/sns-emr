# scripts/import_breast_cancer_production_identity_manifest.py
"""
Breast Cancer Production Identity Manifest v1 -- Verbatim Importer (PR #46).

The first disease-specific Oncology manifest. Reads
backend/manifests/breast_cancer_production_identity_manifest_v1.json (the
...
on top of the existing "Breast Cancer" canonical disease created by the
...
#45):
    ...
    - Resolves (never re-creates) the Oncology body system, the Solid
      Malignancies family, and the existing "Breast Cancer" canonical
    ...
    - Resolves (never re-creates) the four Tier 4 variants the foundation
      already created for Breast Cancer (Breast Primary Site,
      Localized Disease, Metastatic Disease, Recurrent Disease).
    - Creates exactly ONE new Tier 4 variant: "Ductal Carcinoma In Situ"
      (PATHOLOGICAL_SUBTYPE dimension) -- the only variant the approved NCI
      cancer-types catalog supports for this PR.
    - Creates exactly SEVEN Tier 5 FINDING identity concepts: Breast
      Cancer, Ductal Carcinoma In Situ, Male Breast Cancer, Pregnancy and
      Breast Cancer, Localized Breast Cancer, Metastatic Breast Cancer,
      Recurrent Breast Cancer.
    - Creates exactly FOUR explicit, individually-declared Tier4<->Tier5
      applicability mappings (APPLIES_TO): Ductal Carcinoma In Situ concept
      -> its own variant; Localized/Metastatic/Recurrent Breast Cancer
      concepts -> their corresponding foundation variants. NO Cartesian or
      nested-loop mapping generation. "Breast Cancer" (base identity),
      "Male Breast Cancer", and "Pregnancy and Breast Cancer" intentionally
      receive ZERO applicability edges -- there is no approved variant
      dimension for sex/gender/pregnancy "clinical context", and this
      importer refuses to substitute an unrelated dimension for that
      purpose.

This PR does NOT create a universal cancer manifest, does NOT copy any of
this content to another cancer disease, and does NOT create stage, grade,
histology (beyond the one approved PATHOLOGICAL_SUBTYPE variant),
molecular-subtype, metastatic-destination, symptom, treatment, medication,
or prognosis knowledge -- none of that is supported by the approved source
(Cancer-Types-NCI-08.22.2021.pdf) for this PR.

Reuses the exact verbatim-import pattern proven in
scripts/import_oncology_foundation_v1.py (PR #45) and every prior
production-manifest importer (PR #37-#44):
...
Run with: python scripts\\import_breast_cancer_production_identity_manifest.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "breast_cancer_production_identity_manifest_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "breast_cancer_production_identity_acceptance_v1.json"
)
...
# Breast Cancer already carries these Tier 5 FINDING concepts and this one
...
# validation only -- this manifest creates a variant in only ONE of them
...
# are listed here only so differentiation guards can assert their absence.
...
# for this distinction (per the reviewer-approved PR #40/#45 pattern), so it
...
# NCI_CANCER_CATALOG classifies disease *identity* knowledge only (never
# stage/grade/histology/molecular-subtype/treatment/hospice-eligibility);
# ONCOLOGY_FOUNDATION classifies this disease's application of the reusable
# oncology-foundation Localized/Metastatic/Recurrent states. ---
...
def _resolve_existing_disease(db: Session, manifest: dict) -> OntologyDisease:
    ...
    existing Breast Cancer canonical disease -- created by the Oncology
    ...
    writes if any of them is missing, or if more than one Breast Cancer
    ...
    if system is None:
        raise RuntimeError(
            f"Breast Cancer Production Identity Manifest v1 requires the {SYSTEM_NAME!r} body system to "
    ...
    if family is None:
        raise RuntimeError(
            f"Breast Cancer Production Identity Manifest v1 requires the {FAMILY_NAME!r} family to already "
    ...
    if len(matches) == 0:
        raise RuntimeError(
            f"Breast Cancer Production Identity Manifest v1 requires the canonical {DISEASE_NAME!r} disease "
            f"to already exist (created by the Oncology Foundation import, PR #45). Run that import first. "
            f"Aborting without any writes."
        )
    if len(matches) > 1:
        raise RuntimeError(
            f"Breast Cancer Production Identity Manifest v1 found {len(matches)} duplicate {DISEASE_NAME!r} "
    ...
    if disease.disease_family_id != family.id:
        raise RuntimeError(
            f"Breast Cancer Production Identity Manifest v1 found the existing {DISEASE_NAME!r} disease "
...
def _resolve_required_variants(db: Session, manifest: dict, disease: OntologyDisease) -> Dict[Tuple[str, str], OntologyDiseaseVariant]:
    ...
    for v in manifest.get("required_existing_variants", []):
        ...
        if variant is None:
            raise RuntimeError(
                f"Breast Cancer Production Identity Manifest v1 requires the existing {dimension} variant "
...
def _evidence_notes(concept_entry: dict) -> str:
    parts = [
        "Imported verbatim from the approved Breast Cancer Production Identity Manifest v1.",
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, concept_entry: dict) -> bool:
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="breast_cancer_production_identity_manifest_v1",
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Breast Cancer Production Identity Manifest v1 failed validation: {errors}")
    ...
    for concept_type, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for existing in db.query(model_cls).filter_by(disease_id=disease.id).all():
            key = (concept_type, concept_identity_key(concept_type, getattr(existing, name_attr)))
    ...
    for a in manifest.get("applicability", []):
        ...
        if variant_found is None or concept_found is None:
            raise RuntimeError(
                f"Breast Cancer Production Identity Manifest v1 applicability mapping references a "
        ...
        edge = OntologyConceptVariantApplicability(
            ...
            variant_id=variant_found.id,
            applicability_type=applicability_type,
            description="Imported verbatim from the approved Breast Cancer Production Identity Manifest v1.",
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    return {
        "manifest_id": "breast_cancer_production_identity_manifest_v1",
