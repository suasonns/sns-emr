# scripts/import_dementia_production_hardening.py
"""
Dementia Production Hardening v1 -- Verbatim Importer.

Reads backend/manifests/dementia_production_hardening_v1.json
(the sole authoritative source for this import -- never inferred,
reconstructed, or clinically re-derived) and hardens the existing
Neurologic System / Degenerative Brain Disorders / Dementia Due To
Alzheimer's Disease disease (originally created by PR #37) with the
exact source-faithful Tier 4 variants, Tier 5 atomic concepts, and
Tier 4<->Tier 5 applicability mappings this manifest declares:
...
- If the disease/body-system/family already exist (as they do once
  PR #37 is merged), they are resolved and reused -- never duplicated,
  never re-created under a different family.
- Nothing is ever hard-deleted or deactivated. The pre-existing
  non-exact Dementia concepts from PR #37 (e.g. "FAST Stage 7 or
  Beyond", "Urinary and Fecal Incontinence") are left completely
  untouched; this import only adds the new exact-faithful concepts
  alongside them.
...
Critical LCD rule: Dementia diagnosis alone is never sufficient to infer
terminal prognosis or hospice eligibility -- every HOSPICE_ELIGIBILITY_SUPPORT
concept carries its own OntologyEvidenceRule with
patient_fact_requires_evidence = True, and applicability variant identity
is always the explicit triple (disease, variant_dimension, variant_name) --
never name-only matching. Per the CMS Dementia LCD, FAST Stage 7 plus any
one qualifying comorbidity/dependency finding establishes eligibility, so
every one of the 16 exact source-faithful concepts also carries a
dual-domain HOSPICE_ELIGIBILITY_SUPPORT entry -- but the diagnosis-only
variant never receives HOSPICE_SUPPORT_FOR applicability, only the FAST
Stage 7 variant does. The exact functional/dependency findings (FAST
Stage 7, Unable To Ambulate/Dress/Bathe Without Assistance, Urinary
Incontinence, Fecal Incontinence, No Consistently Meaningful Verbal
Communication) require documented functional-assessment evidence, the
exact comorbidity findings (Aspiration Pneumonia, Pyelonephritis, Upper
Urinary Tract Infection, Septicemia, Stage 3/4 Pressure Ulcer, Recurrent
Fever After Antibiotics) require documented diagnosis-date evidence, the
exact nutritional finding (Ten Percent Weight Loss In Six Months)
requires documented weight-data evidence, and the exact laboratory
threshold (Serum Albumin Less Than 2.5 g/dL) requires laboratory_name,
laboratory_value, laboratory_unit, laboratory_date, and source_record --
none of these is ever inferred.

Run with: .\\.venv\\Scripts\\python.exe scripts\\import_dementia_production_hardening.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "dementia_production_hardening_v1.json"
)
...
# uses only a subset (DISEASE_PHASE, STAGE). ---
...
def _resolve_or_create_diseases(db: Session, manifest: dict) -> Dict[str, OntologyDisease]:
    ...
    Neurologic System body system, the declared family, and the disease
    itself if not already present -- but resolving and reusing them
    unchanged if they already exist (as they do once PR #37 is merged).
    Never creates a new body system/family/disease beyond what the
...
def _evidence_notes(concept_entry: dict) -> str:
    ...
    parts = [
        "Imported verbatim from the approved Dementia Production Hardening v1.",
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, concept_entry: dict | None = None) -> bool:
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="dementia_production_hardening_v1",
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Dementia Production Hardening v1 failed structural/vocabulary validation: {errors}")
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for v in disease_entry.get("variants", []):
            ...
            variant = OntologyDiseaseVariant(
                ...
                source_reference="dementia_production_hardening_v1",
            )
            db.add(variant)
            db.flush()
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for a in disease_entry.get("applicability", []):
            ...
            if variant_key_found is None or concept_key_found is None:
                raise RuntimeError(
                    f"Dementia Production Hardening v1 applicability mapping references a variant/concept "
            ...
            edge = OntologyConceptVariantApplicability(
                ...
                variant_id=variant_key_found.id,
                applicability_type=applicability_type,
                description="Imported verbatim from the approved Dementia Production Hardening v1.",
                evidence_requirement=(
