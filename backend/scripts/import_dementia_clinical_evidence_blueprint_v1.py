# scripts/import_dementia_clinical_evidence_blueprint_v1.py
"""
Dementia Clinical Evidence Blueprint v1 -- Extension-Only Importer.

Reads backend/manifests/dementia_clinical_evidence_blueprint_v1.json (the
sole authoritative source for this import) and EXTENDS the existing
"Dementia Due To Alzheimer's Disease" disease created by the Dementia
Production Hardening manifest (PR #44, merged). This importer:

- NEVER creates a new OntologyBodySystem, OntologyDiseaseFamily, or
  OntologyDisease row. If "Dementia Due To Alzheimer's Disease" or the
  "Functional Assessment Framework" disease (PR #49, merged) does not
  already exist, the import aborts before any writes.
- NEVER creates a new OntologyDiseaseVariant row (no new FAST stages).
- NEVER duplicates PR #44's or PR #49's FAST/PPS/KPS definitions, and
  NEVER duplicates a major complication already present in PR #44 (see
  the manifest's already_present_verified.mapping). Instead it adds
  three OntologyRelationship edges linking Dementia's existing FAST
  Stage 7 / PPS Less Than 70 Percent / KPS Less Than 70 Percent concepts
  to the Functional Assessment Framework's existing FAST/PPS/KPS
  OntologyDiseaseFinding rows.
- Adds a small set of genuinely missing atomic Dementia concepts (a
  finding, functional impacts, nutritional impacts, a complication) and
  10 Dementia recertification-trend PROGNOSTIC_INDICATOR concepts, every
...
Run with: .\\.venv\\Scripts\\python.exe scripts\\import_dementia_clinical_evidence_blueprint_v1.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "dementia_clinical_evidence_blueprint_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "dementia_clinical_evidence_blueprint_acceptance_v1.json"
)

EXTENDS_DISEASE_NAME = "Dementia Due To Alzheimer's Disease"
FAF_DISEASE_NAME = "Functional Assessment Framework"

ALLOWED_CONCEPT_DOMAINS = {"FINDING", "NUTRITIONAL_IMPACT", "COMPLICATION", "FUNCTIONAL_IMPACT"}
...
# already-existing Dementia/FAF concepts referenced by name (never to create).
LOOKUP_DOMAIN_MODEL_MAP = dict(CONCEPT_DOMAIN_MODEL_MAP)
LOOKUP_DOMAIN_MODEL_MAP["HOSPICE_ELIGIBILITY_SUPPORT"] = (
    OntologyDiseaseHospiceEligibilitySupport, "indicator_name"
)
...
def validate_manifest(manifest: dict) -> List[str]:
    ...
    if scope.get("extends_disease") != EXTENDS_DISEASE_NAME:
        errors.append("manifest scope.extends_disease must be 'Dementia Due To Alzheimer's Disease'")
    ...
    if recert.get("disease") != EXTENDS_DISEASE_NAME:
        errors.append("recertification_evidence_model.disease must be 'Dementia Due To Alzheimer's Disease'")
    seen_trend = set()
...
def _resolve_disease(db: Session, name: str) -> OntologyDisease:
    ...
    if disease is None:
        raise RuntimeError(
            f"Dementia Clinical Evidence Blueprint v1 requires the disease {name!r} to already exist. "
...
def _resolve_variant(db: Session, disease_id, variant_name: str) -> OntologyDiseaseVariant:
    ...
    if variant is None:
        raise RuntimeError(
            f"Dementia Clinical Evidence Blueprint v1 references variant {variant_name!r} which was not "
            "found on the existing Dementia Due To Alzheimer's Disease disease (PR #44). Aborting without any writes."
        )
    return variant
...
def _resolve_concept(db: Session, disease_id, domain: str, name: str):
    ...
    if row is None:
        raise RuntimeError(
            f"Dementia Clinical Evidence Blueprint v1 references concept {name!r} (domain={domain}) which "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, content_source_type: str,
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="dementia_clinical_evidence_blueprint_v1",
            ...
            notes=_provenance_text(
                ...
                f"Imported from the approved Dementia Clinical Evidence Blueprint v1. {description}",
            ),
        )
    )
    return True
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Dementia Clinical Evidence Blueprint v1 failed structural/vocabulary validation: {errors}")

    dementia_disease = _resolve_disease(db, EXTENDS_DISEASE_NAME)
    ...
    for domain, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for existing in db.query(model_cls).filter_by(disease_id=dementia_disease.id).all():
    ...
    for c in manifest.get("new_concepts", []):
        ...
        if key in concept_by_key:
            ...
            description_attr = "finding_description" if domain == "FINDING" else "description"
            row = model_cls(
                id=uuid.uuid4(),
                disease_id=dementia_disease.id,
    ...
    # --- new applicability edges (new concept -> existing Dementia variant) ---
    for a in manifest.get("new_applicability", []):
        ...
        variant_row = _resolve_variant(db, dementia_disease.id, variant_name)
        concept_key = (concept_domain, concept_name.strip().lower())
        if concept_key not in concept_by_key:
            raise RuntimeError(
                f"Dementia Clinical Evidence Blueprint v1 applicability mapping references a concept that "
        ...
        db.add(
            OntologyConceptVariantApplicability(
                ...
                variant_id=variant_row.id,
                applicability_type=applicability_type,
                description="Imported from the approved Dementia Clinical Evidence Blueprint v1.",
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        dementia_concept = _resolve_concept(
            db, dementia_disease.id, link["chf_concept_domain"], link["chf_concept_name"]
        )
        ...
        existing_rel = (
            ...
            .filter_by(
                ...
                source_concept_id=dementia_concept.id,
    ...
    # --- recertification trend indicators (PROGNOSTIC_INDICATOR, Dementia-scoped) ---
    existing_trend = {
        row.indicator_name.strip().lower(): row
        for row in db.query(OntologyDiseasePrognosticIndicator).filter_by(disease_id=dementia_disease.id).all()
    }
    recert = manifest.get("recertification_evidence_model", {})
    for t in recert.get("trend_indicators", []):
        name = t["name"]
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    for domain, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for row in db.query(model_cls).filter_by(disease_id=dementia_disease.id).all():
    ...
    # concepts in the same shared domains that belong to PR #44's Dementia
    # foundation, not to this manifest -- informational only, never a
    # validation failure
    pre_existing_pr44_concepts = sorted(
    ...
    stored_trend_rows = db.query(OntologyDiseasePrognosticIndicator).filter_by(disease_id=dementia_disease.id).all()
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        ...
        dementia_concept = _resolve_concept(db, dementia_disease.id, link["chf_concept_domain"], link["chf_concept_name"])
    ...
    for a in manifest.get("new_applicability", []):
        variant = _resolve_variant(db, dementia_disease.id, a["variant"])
    ...
    # provenance coverage across only the concepts/trend indicators this
    # manifest is responsible for -- never PR #44's pre-existing content
    ...
    return {
        ...
        "concepts": {
            ...
            "pre_existing_pr44_concepts_in_shared_domains": pre_existing_pr44_concepts,
        },
        "applicability": {
