# scripts/import_als_clinical_evidence_blueprint_v1.py
"""
ALS Clinical Evidence Blueprint v1 -- Extension-Only Importer.

Reads backend/manifests/als_clinical_evidence_blueprint_v1.json (the sole
authoritative source for this import) and EXTENDS the existing
"Amyotrophic Lateral Sclerosis" disease created by the ALS Production
Knowledge Manifest v1 (PR #43, merged). This importer:

- NEVER creates a new OntologyBodySystem, OntologyDiseaseFamily, or
  OntologyDisease row. If "Amyotrophic Lateral Sclerosis" or the
...
- NEVER duplicates PR #43's or PR #49's PPS/KPS/ADL definitions. Instead
  it adds two OntologyRelationship edges linking ALS's existing PPS Less
  Than 70 Percent / KPS Less Than 70 Percent concepts to the Functional
  Assessment Framework's existing PPS/KPS OntologyDiseaseFinding rows.
- Adds a small set of missing atomic ALS concepts (5 COMPLICATION
  co-morbidities/complications, 2 HOSPICE_ELIGIBILITY_SUPPORT
  treatment-decision indicators) and 9 ALS recertification-trend
  PROGNOSTIC_INDICATOR concepts, every one carrying provenance metadata
  (content_source_type, content_review_status) encoded verbatim into the
  existing OntologyEvidenceRule.notes free-text field -- no schema,
  migration, or API changes.
- Idempotent: re-running inserts nothing new.
- Nothing is ever hard-deleted or deactivated.

Run with: .\\.venv\\Scripts\\python.exe scripts\\import_als_clinical_evidence_blueprint_v1.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "als_clinical_evidence_blueprint_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "als_clinical_evidence_blueprint_acceptance_v1.json"
)

EXTENDS_DISEASE_NAME = "Amyotrophic Lateral Sclerosis"
FAF_DISEASE_NAME = "Functional Assessment Framework"

ALLOWED_CONCEPT_DOMAINS = {
    "SYMPTOM", "NUTRITIONAL_IMPACT", "COMPLICATION", "FUNCTIONAL_IMPACT",
...
# already-existing ALS/FAF concepts referenced by name (never to create).
...
def validate_manifest(manifest: dict) -> List[str]:
    ...
    if scope.get("extends_disease") != EXTENDS_DISEASE_NAME:
        errors.append("manifest scope.extends_disease must be 'Amyotrophic Lateral Sclerosis'")
    ...
    if recert.get("disease") != EXTENDS_DISEASE_NAME:
        errors.append("recertification_evidence_model.disease must be 'Amyotrophic Lateral Sclerosis'")
    seen_trend = set()
...
def _resolve_disease(db: Session, name: str) -> OntologyDisease:
    ...
    if disease is None:
        raise RuntimeError(
            f"ALS Clinical Evidence Blueprint v1 requires the disease {name!r} to already exist. "
...
def _resolve_variant(db: Session, disease_id, variant_name: str) -> OntologyDiseaseVariant:
    ...
    if variant is None:
        raise RuntimeError(
            f"ALS Clinical Evidence Blueprint v1 references variant {variant_name!r} which was not "
            "found on the existing Amyotrophic Lateral Sclerosis disease (PR #43). Aborting without any writes."
        )
    return variant
...
def _resolve_concept(db: Session, disease_id, domain: str, name: str):
    ...
    if row is None:
        raise RuntimeError(
            f"ALS Clinical Evidence Blueprint v1 references concept {name!r} (domain={domain}) which "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, content_source_type: str,
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="als_clinical_evidence_blueprint_v1",
            ...
            notes=_provenance_text(
                ...
                f"Imported from the approved ALS Clinical Evidence Blueprint v1. {description}",
            ),
        )
    )
    return True
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"ALS Clinical Evidence Blueprint v1 failed structural/vocabulary validation: {errors}")

    als_disease = _resolve_disease(db, EXTENDS_DISEASE_NAME)
    ...
    # index existing concepts for the ALS disease, across the small set of
    ...
    for domain, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for existing in db.query(model_cls).filter_by(disease_id=als_disease.id).all():
    ...
    # --- new applicability edges (new concept -> existing ALS variant) ---
    for a in manifest.get("new_applicability", []):
        ...
        variant_row = _resolve_variant(db, als_disease.id, variant_name)
        concept_key = (concept_domain, concept_name.strip().lower())
        if concept_key not in concept_by_key:
            raise RuntimeError(
                f"ALS Clinical Evidence Blueprint v1 applicability mapping references a concept that "
        ...
        db.add(
            OntologyConceptVariantApplicability(
                ...
                variant_id=variant_row.id,
                applicability_type=applicability_type,
                description="Imported from the approved ALS Clinical Evidence Blueprint v1.",
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        chf_concept = _resolve_concept(
            db, als_disease.id, link["chf_concept_domain"], link["chf_concept_name"]
        )
    ...
    # --- recertification trend indicators (PROGNOSTIC_INDICATOR, ALS-scoped) ---
    existing_trend = {
        row.indicator_name.strip().lower(): row
        for row in db.query(OntologyDiseasePrognosticIndicator).filter_by(disease_id=als_disease.id).all()
    }
    recert = manifest.get("recertification_evidence_model", {})
    for t in recert.get("trend_indicators", []):
        name = t["name"]
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    for domain, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for row in db.query(model_cls).filter_by(disease_id=als_disease.id).all():
    ...
    # concepts in the same shared domains that belong to PR #43's ALS foundation,
    # not to this manifest -- informational only, never a validation failure
    pre_existing_pr38_concepts = sorted(
    ...
    stored_trend_rows = db.query(OntologyDiseasePrognosticIndicator).filter_by(disease_id=als_disease.id).all()
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        ...
        chf_concept = _resolve_concept(db, als_disease.id, link["chf_concept_domain"], link["chf_concept_name"])
    ...
    for a in manifest.get("new_applicability", []):
        variant = _resolve_variant(db, als_disease.id, a["variant"])
    ...
    return {
        ...
        "concepts": {
            ...
            "pre_existing_pr38_concepts_in_shared_domains": pre_existing_pr38_concepts,
        },
        "applicability": {
