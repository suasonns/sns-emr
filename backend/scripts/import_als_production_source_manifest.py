# scripts/import_als_production_source_manifest.py
"""
ALS Production Source Manifest v1 -- Verbatim Importer.

Reads backend/manifests/als_production_source_manifest_v1.json
(the sole authoritative source for this import -- never inferred,
reconstructed, or clinically re-derived) and creates the Neuromuscular
System body system / family, the single canonical disease, every Tier 4
...
scripts/import_liver_production_source_manifest.py (PR #41), and
scripts/import_hiv_production_source_manifest.py (PR #42):
...
Critical LCD rule: ALS diagnosis alone is never sufficient to infer
...
never name-only matching. The exact respiratory findings (Critically
Impaired Breathing Capacity, Vital Capacity Decline, Respiratory
Insufficiency, Dependence On Ventilatory Support) require documented
respiratory-measurement evidence (respiratory_measurement, date,
source_record) and the exact nutritional findings (Severe Nutritional
Impairment, Significant Weight Loss, Reduced Oral Intake, Dysphagia)
require documented weight/intake evidence (weight_data, intake_data,
source_record) folded into their evidence rules -- neither is ever
inferred.

Run with: .\\.venv\\Scripts\\python.exe scripts\\import_als_production_source_manifest.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "als_production_source_manifest_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "als_production_manifest_acceptance_v1.json"
)
IMPORTER_NAME = "import_als_production_source_manifest"


SYSTEM_NAME = "Neuromuscular System"
...
# RECURRENCE_STATE, PHYSIOLOGICAL_PHENOTYPE). ---
...
def _resolve_or_create_diseases(db: Session, manifest: dict) -> Dict[str, OntologyDisease]:
    ...
    Neuromuscular System body system, the declared family, and the disease
    ...
    if len(family_names) != 1:
        raise RuntimeError(
            f"ALS Production Manifest v1 declares {len(family_names)} families; "
...
def _evidence_notes(concept_entry: dict) -> str:
    ...
    parts = [
        "Imported verbatim from the approved ALS Production Source Manifest v1.",
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, concept_entry: dict | None = None) -> bool:
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="als_production_source_manifest_v1",
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"ALS Production Manifest v1 failed structural/vocabulary validation: {errors}")
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for v in disease_entry.get("variants", []):
            ...
            variant = OntologyDiseaseVariant(
                ...
                source_reference="als_production_source_manifest_v1",
            )
            db.add(variant)
            db.flush()
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for a in disease_entry.get("applicability", []):
            ...
            if variant_key_found is None or concept_key_found is None:
                raise RuntimeError(
                    f"ALS Production Manifest v1 applicability mapping references a variant/concept "
            ...
            edge = OntologyConceptVariantApplicability(
                ...
                variant_id=variant_key_found.id,
                applicability_type=applicability_type,
                description="Imported verbatim from the approved ALS Production Source Manifest v1.",
                evidence_requirement=(
