# scripts/import_hiv_clinical_evidence_blueprint_v1.py
"""
HIV Clinical Evidence Blueprint v1 -- Extension-Only Importer.

Reads backend/manifests/hiv_clinical_evidence_blueprint_v1.json (the
sole authoritative source for this import) and EXTENDS the existing
"Advanced HIV Disease" AND "AIDS" diseases created by the HIV Production
Knowledge Manifest v1 (PR #42, merged). Like the Liver Clinical Evidence
Blueprint (and unlike the Renal Clinical Evidence Blueprint, which
extends two genuinely distinct diseases asymmetrically), this manifest
applies every new concept IDENTICALLY to both HIV diseases -- PR #42
built them with an identical LAB/COMPLICATION/HOSPICE_ELIGIBILITY_SUPPORT
concept set, so extending only one would create ontology drift between
them. Unlike Liver, however, the two HIV diseases do NOT share identical
DISEASE_PHASE variant names ("Terminal Advanced HIV Disease" vs.
"Terminal AIDS"), so every new_applicability entry declares a
variant_by_disease mapping (disease name -> that disease's own variant
name) instead of a single shared variant string. This importer:

- NEVER creates a new OntologyBodySystem, OntologyDiseaseFamily, or
  OntologyDisease row. If either "Advanced HIV Disease", "AIDS", or
  "Functional Assessment Framework" (PR #49, merged) does not already
  exist, the import aborts before any writes.
- NEVER creates a new OntologyDiseaseVariant row.
- NEVER duplicates PR #42's or PR #49's PPS/KPS definitions, and NEVER
  duplicates a lab/complication/hospice-eligibility-support concept
  already present in PR #42 (see the manifest's
...
- Adds five genuinely missing atomic HIV/AIDS concepts (Recurrent
  Bacterial Sepsis, Recurrent Aspiration Pneumonia, Persistent Diarrhea
  With Significant Weight Loss, Antiretroviral Therapy Refused Or
  Discontinued, Concurrent Life-Threatening Comorbid Condition), applied
  identically to both diseases, and 9 HIV recertification-trend
  PROGNOSTIC_INDICATOR concepts per disease, every one carrying
  provenance metadata (content_source_type, content_review_status)
  encoded verbatim into the existing OntologyEvidenceRule.notes free-text
  field -- no schema, migration, or API changes.
- Idempotent: re-running inserts nothing new.
- Nothing is ever hard-deleted or deactivated.

Run with: .\\.venv\\Scripts\\python.exe scripts\\import_hiv_clinical_evidence_blueprint_v1.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "hiv_clinical_evidence_blueprint_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "hiv_clinical_evidence_blueprint_acceptance_v1.json"
)

EXTENDS_DISEASE_NAMES = ["Advanced HIV Disease", "AIDS"]
...
# already-existing HIV/FAF concepts referenced by name (never to create).
...
def validate_manifest(manifest: dict) -> List[str]:
    ...
    for a in manifest.get("new_applicability", []):
        ...
        if not isinstance(variant_by_disease, dict) or not a.get("concept"):
            errors.append("new_applicability entry missing variant_by_disease (dict) or concept")
        else:
            missing_diseases = [name for name in EXTENDS_DISEASE_NAMES if not variant_by_disease.get(name)]
            if missing_diseases:
                errors.append(
                    f"new_applicability entry variant_by_disease is missing an entry for: {missing_diseases}"
                )
        if not a.get("variant_dimension"):
            errors.append("new_applicability entry missing variant_dimension (required -- HIV/AIDS variant "
...
def _resolve_disease(db: Session, name: str) -> OntologyDisease:
    ...
    if disease is None:
        raise RuntimeError(
            f"HIV Clinical Evidence Blueprint v1 requires the disease {name!r} to already exist. "
...
def _resolve_variant(db: Session, disease_id, variant_name: str, variant_dimension: str | None = None) -> OntologyDiseaseVariant:
    ...
    is provided. A dimension is REQUIRED whenever the HIV/AIDS disease has
    more than one variant sharing the same name across dimensions -- omitting
    it in that case raises rather than silently picking one."""
    ...
    if len(matches) == 0:
        raise RuntimeError(
            f"HIV Clinical Evidence Blueprint v1 references variant {variant_name!r} "
            f"(dimension={variant_dimension!r}) which was not found on the existing HIV/AIDS disease. "
            "Aborting without any writes."
        )
    if len(matches) > 1:
        raise RuntimeError(
            f"HIV Clinical Evidence Blueprint v1 references variant {variant_name!r} which is "
...
def _resolve_concept(db: Session, disease_id, domain: str, name: str):
    ...
    if row is None:
        raise RuntimeError(
            f"HIV Clinical Evidence Blueprint v1 references concept {name!r} (domain={domain}) which "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, content_source_type: str,
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="hiv_clinical_evidence_blueprint_v1",
            ...
            notes=_provenance_text(
                ...
                f"Imported from the approved HIV Clinical Evidence Blueprint v1. {description}",
            ),
        )
    )
    return True
...
def _run_for_disease(db: Session, manifest: dict, disease: OntologyDisease, faf_disease: OntologyDisease) -> dict:
    ...
    single already-existing HIV/AIDS disease. Called once per disease in
    ...
    # --- new applicability edges (new concept -> existing HIV/AIDS variant) ---
    for a in manifest.get("new_applicability", []):
        variant_name = a["variant_by_disease"][disease.disease_name]
        ...
        if concept_key not in concept_by_key:
            raise RuntimeError(
                f"HIV Clinical Evidence Blueprint v1 applicability mapping references a concept that "
        ...
        db.add(
            OntologyConceptVariantApplicability(
                ...
                variant_id=variant_row.id,
                applicability_type=applicability_type,
                description="Imported from the approved HIV Clinical Evidence Blueprint v1.",
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        hiv_concept = _resolve_concept(db, disease.id, link["chf_concept_domain"], link["chf_concept_name"])
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"HIV Clinical Evidence Blueprint v1 failed structural/vocabulary validation: {errors}")
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    for disease in diseases:
        ...
        for a in expected_applicability:
            variant = _resolve_variant(db, disease.id, a["variant_by_disease"][disease.disease_name], a.get("variant_dimension"))
