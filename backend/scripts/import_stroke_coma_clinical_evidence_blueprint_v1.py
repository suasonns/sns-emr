# scripts/import_stroke_coma_clinical_evidence_blueprint_v1.py
"""
Stroke/Coma Clinical Evidence Blueprint v1 -- Extension-Only Importer.

Reads backend/manifests/stroke_coma_clinical_evidence_blueprint_v1.json (the
sole authoritative source for this import) and EXTENDS the existing
"Stroke" disease created by the Neurologic Production Source Manifest v1
(PR #37, merged). Coma is NOT a separate disease -- it already exists
inside Stroke as END_STAGE_FINDING concepts ("Coma", "Persistent Coma",
"Persistent Vegetative State", "Minimally Conscious State") and as the
SEVERITY_CLASS variant "Stroke With Persistent Coma". This importer:

- NEVER creates a new OntologyBodySystem, OntologyDiseaseFamily, or
  OntologyDisease row. If "Stroke" or the "Functional Assessment
  Framework" disease (PR #49, merged) does not already exist, the import
  aborts before any writes.
- NEVER creates a new OntologyDiseaseVariant row.
- NEVER duplicates PR #37's or PR #49's PPS/KPS/ADL definitions. Instead
  it adds two OntologyRelationship edges linking Stroke's existing
  Palliative Performance Scale 40 Percent or Less / Karnofsky Performance
  Status 40 Percent or Less concepts to the Functional Assessment
  Framework's existing PPS/KPS OntologyDiseaseFinding rows.
- Adds a small set of missing universal HOSPICE_ELIGIBILITY_SUPPORT
  concepts (ADL Dependence, Progressive Functional Decline, Progressive
  Nutritional Decline) that every other disease foundation already
  carries but Stroke's foundation never received, links them to the
  existing "Stroke With Persistent Coma" variant via new
  HOSPICE_SUPPORT_FOR applicability edges, and adds 7 Stroke
  recertification-trend PROGNOSTIC_INDICATOR concepts, every one
  carrying provenance metadata (content_source_type,
...
Run with: .\\.venv\\Scripts\\python.exe scripts\\import_stroke_coma_clinical_evidence_blueprint_v1.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "stroke_coma_clinical_evidence_blueprint_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "stroke_coma_clinical_evidence_blueprint_acceptance_v1.json"
)

EXTENDS_DISEASE_NAME = "Stroke"
...
# concept_type values used by the FAF importer's OntologyDiseaseFinding rows
...
# already-existing Stroke/FAF concepts referenced by name (never to create).
LOOKUP_DOMAIN_MODEL_MAP = dict(CONCEPT_DOMAIN_MODEL_MAP)
LOOKUP_DOMAIN_MODEL_MAP["FINDING"] = (OntologyDiseaseFinding, "finding_name")
...
def validate_manifest(manifest: dict) -> List[str]:
    ...
    if scope.get("extends_disease") != EXTENDS_DISEASE_NAME:
        errors.append("manifest scope.extends_disease must be 'Stroke'")
    ...
    if recert.get("disease") != EXTENDS_DISEASE_NAME:
        errors.append("recertification_evidence_model.disease must be 'Stroke'")
    seen_trend = set()
...
def _resolve_disease(db: Session, name: str) -> OntologyDisease:
    ...
    if disease is None:
        raise RuntimeError(
            f"Stroke/Coma Clinical Evidence Blueprint v1 requires the disease {name!r} to already exist. "
...
def _resolve_variant(db: Session, disease_id, variant_name: str) -> OntologyDiseaseVariant:
    normalized = variant_name.strip().lower()
    variant = (
        db.query(OntologyDiseaseVariant)
        .filter_by(disease_id=disease_id, normalized_name=normalized)
        .one_or_none()
    )
    if variant is None:
        raise RuntimeError(
            f"Stroke/Coma Clinical Evidence Blueprint v1 references variant {variant_name!r} which was not "
            "found on the existing Stroke disease (PR #37). Aborting without any writes."
        )
    return variant
...
def _resolve_concept(db: Session, disease_id, domain: str, name: str):
    ...
    if row is None:
        raise RuntimeError(
            f"Stroke/Coma Clinical Evidence Blueprint v1 references concept {name!r} (domain={domain}) which "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, content_source_type: str,
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="stroke_coma_clinical_evidence_blueprint_v1",
            ...
            notes=_provenance_text(
                ...
                f"Imported from the approved Stroke/Coma Clinical Evidence Blueprint v1. {description}",
            ),
        )
    )
    return True
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Stroke/Coma Clinical Evidence Blueprint v1 failed structural/vocabulary validation: {errors}")

    stroke_disease = _resolve_disease(db, EXTENDS_DISEASE_NAME)
    ...
    # index existing concepts for the Stroke disease, across the small set of
    # domains this manifest is allowed to add to
    concept_by_key: Dict[Tuple[str, str], object] = {}
    for domain, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for existing in db.query(model_cls).filter_by(disease_id=stroke_disease.id).all():
    ...
    # --- new applicability edges (new concept -> existing Stroke variant) ---
    for a in manifest.get("new_applicability", []):
        ...
        variant_row = _resolve_variant(db, stroke_disease.id, variant_name)
        concept_key = (concept_domain, concept_name.strip().lower())
        if concept_key not in concept_by_key:
            raise RuntimeError(
                f"Stroke/Coma Clinical Evidence Blueprint v1 applicability mapping references a concept that "
        ...
        db.add(
            OntologyConceptVariantApplicability(
                ...
                variant_id=variant_row.id,
                applicability_type=applicability_type,
                description="Imported from the approved Stroke/Coma Clinical Evidence Blueprint v1.",
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        chf_concept = _resolve_concept(
            db, stroke_disease.id, link["chf_concept_domain"], link["chf_concept_name"]
        )
        faf_concept = _resolve_concept(
            db, faf_disease.id, FAF_LINKAGE_TARGET_DOMAIN, link["faf_concept_name"]
        )
    ...
    # --- recertification trend indicators (PROGNOSTIC_INDICATOR, Stroke-scoped) ---
    existing_trend = {
        row.indicator_name.strip().lower(): row
        for row in db.query(OntologyDiseasePrognosticIndicator).filter_by(disease_id=stroke_disease.id).all()
    }
    recert = manifest.get("recertification_evidence_model", {})
    for t in recert.get("trend_indicators", []):
        name = t["name"]
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    and report expected vs. stored for new concepts, applicability,
    FAF relationships, and trend indicators, plus provenance coverage
    and the second-run new-row count. Never a clinical judgment --
    purely a mechanical comparison."""
    stroke_disease = _resolve_disease(db, EXTENDS_DISEASE_NAME)
    faf_disease = _resolve_disease(db, FAF_DISEASE_NAME)
    ...
    for domain, (model_cls, name_attr) in CONCEPT_DOMAIN_MODEL_MAP.items():
        for row in db.query(model_cls).filter_by(disease_id=stroke_disease.id).all():
    ...
    # concepts in the same shared domains that belong to PR #37's Stroke foundation,
    # not to this manifest -- informational only, never a validation failure
    pre_existing_pr37_concepts = sorted(
        k for k in (stored_concept_keys - expected_concept_keys)
        if k[0] in CONCEPT_DOMAIN_MODEL_MAP
    )

    expected_trend_keys = {
        t["name"].strip().lower() for t in manifest.get("recertification_evidence_model", {}).get("trend_indicators", [])
    }
    stored_trend_rows = db.query(OntologyDiseasePrognosticIndicator).filter_by(disease_id=stroke_disease.id).all()
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        ...
        chf_concept = _resolve_concept(db, stroke_disease.id, link["chf_concept_domain"], link["chf_concept_name"])
    ...
    expected_applicability = len(manifest.get("new_applicability", []))
    stored_applicability = 0
    for a in manifest.get("new_applicability", []):
        variant = _resolve_variant(db, stroke_disease.id, a["variant"])
        ...
        if exists is not None:
            stored_applicability += 1

    # provenance coverage across every evidence rule this manifest is responsible for
    ...
    return {
        ...
        "extends_disease": EXTENDS_DISEASE_NAME,
        "extends_functional_assessment_framework": FAF_DISEASE_NAME,
        "concepts": {
            "expected": len(expected_concept_keys),
            "stored": len(stored_concept_keys & expected_concept_keys),
            "missing": missing_concepts,
            "pre_existing_pr37_concepts_in_shared_domains": pre_existing_pr37_concepts,
        },
        "applicability": {
            "expected": expected_applicability,
            "stored": stored_applicability,
        },
        "functional_assessment_relationships": {
            "expected": expected_relationships,
            "stored": stored_relationships,
        },
        "recertification_trend_indicators": {
            "expected": len(expected_trend_keys),
            "stored": len(stored_trend_keys & expected_trend_keys),
            "missing": missing_trend,
        },
        "provenance_coverage": {
