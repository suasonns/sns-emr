# scripts/import_renal_clinical_evidence_blueprint_v1.py
"""
Renal Clinical Evidence Blueprint v1 -- Extension-Only Importer.

Reads backend/manifests/renal_clinical_evidence_blueprint_v1.json (the sole
authoritative source for this import) and EXTENDS the existing "Acute Renal
Failure" and "Chronic Renal Failure" diseases created by the Renal
Production Source Manifest (PR #40, merged). Unlike the Pulmonary Clinical
Evidence Blueprint (which extends two IDENTICAL mirrored diseases
symmetrically), Acute Renal Failure and Chronic Renal Failure are genuinely
DISTINCT disease identities with different existing concept sets -- this
importer therefore does NOT force symmetry: each new_concepts entry
declares its own "diseases" list naming exactly which disease(s) it
applies to, and each new_applicability entry declares its own "disease".

This importer:

- NEVER creates a new OntologyBodySystem, OntologyDiseaseFamily, or
  OntologyDisease row. If "Acute Renal Failure", "Chronic Renal Failure",
  or "Functional Assessment Framework" (PR #49, merged) does not already
  exist, the import aborts before any writes.
- NEVER creates a new OntologyDiseaseVariant row.
- NEVER duplicates PR #40's or PR #49's PPS/KPS definitions, and NEVER
  duplicates a concept already present in either renal disease (see the
  manifest's already_present_verified.mapping). Instead it adds two
  ...
  Functional Assessment Framework's existing PPS/KPS OntologyDiseaseFinding
  rows.
- Adds 5 genuinely missing atomic FINDING concepts (Uremia, Oliguria,
  Intractable Hyperkalemia, Uremic Pericarditis, Hepatorenal Syndrome) to
  Acute Renal Failure ONLY, plus 1 new HOSPICE_ELIGIBILITY_SUPPORT concept
  (Not Candidate For Dialysis) applied to BOTH diseases, and 9 renal
...
Run with: .\\.venv\\Scripts\\python.exe scripts\\import_renal_clinical_evidence_blueprint_v1.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "renal_clinical_evidence_blueprint_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "renal_clinical_evidence_blueprint_acceptance_v1.json"
)

EXTENDS_DISEASE_NAMES = ["Acute Renal Failure", "Chronic Renal Failure"]
# Kept singular for compatibility with the shared test-suite pattern used by
# every other Clinical Evidence Blueprint importer (CHF, Dementia, Pulmonary).
...
# already-existing Renal/FAF concepts referenced by name (never to create).
...
def validate_manifest(manifest: dict) -> List[str]:
    ...
    judgment. Confirms every new concept, applicability edge, FAF linkage,
    and recertification trend indicator declares valid, present provenance
    metadata (ALL_CLINICAL_CONTENT_HAS_PROVENANCE), and that the manifest
    never declares a new disease/variant."""
    errors: List[str] = []
    ...
    for c in manifest.get("new_concepts", []):
        ...
        if not diseases or not isinstance(diseases, list) or not set(diseases).issubset(set(EXTENDS_DISEASE_NAMES)):
            errors.append(f"new_concepts entry {name!r} must declare a non-empty diseases list drawn from {EXTENDS_DISEASE_NAMES}")
    ...
    for a in manifest.get("new_applicability", []):
        if a.get("disease") not in EXTENDS_DISEASE_NAMES:
            errors.append(f"new_applicability entry has invalid disease: {a.get('disease')!r}")
        ...
        if not a.get("variant_dimension"):
            errors.append("new_applicability entry missing variant_dimension (required -- renal variant "
                           "names are not unique across dimensions, e.g. 'Terminal Acute Renal Failure')")
...
def _resolve_disease(db: Session, name: str) -> OntologyDisease:
    ...
    if disease is None:
        raise RuntimeError(
            f"Renal Clinical Evidence Blueprint v1 requires the disease {name!r} to already exist. "
...
def _resolve_variant(db: Session, disease_id, variant_name: str, variant_dimension: str | None = None) -> OntologyDiseaseVariant:
    ...
    is provided. A dimension is REQUIRED whenever the renal disease has more
    than one variant sharing the same name across dimensions (e.g. 'Terminal
    Acute Renal Failure' exists as both a DISEASE_PHASE and a SEVERITY_CLASS
    variant) -- omitting it in that case raises rather than silently
    picking one."""
    ...
    if len(matches) == 0:
        raise RuntimeError(
            f"Renal Clinical Evidence Blueprint v1 references variant {variant_name!r} "
            f"(dimension={variant_dimension!r}) which was not found on the existing renal disease. "
            "Aborting without any writes."
        )
    if len(matches) > 1:
        raise RuntimeError(
            f"Renal Clinical Evidence Blueprint v1 references variant {variant_name!r} which is "
...
def _resolve_concept(db: Session, disease_id, domain: str, name: str):
    ...
    if row is None:
        raise RuntimeError(
            f"Renal Clinical Evidence Blueprint v1 references concept {name!r} (domain={domain}) which "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, content_source_type: str,
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="renal_clinical_evidence_blueprint_v1",
            ...
            notes=_provenance_text(
                ...
                f"Imported from the approved Renal Clinical Evidence Blueprint v1. {description}",
            ),
        )
    )
    return True
...
def _run_for_disease(db: Session, manifest: dict, disease: OntologyDisease, faf_disease: OntologyDisease) -> dict:
    """Applies the subset of the manifest's new_concepts / new_applicability
    that name this disease, plus the disease-agnostic functional_assessment_
    linkage and recertification trend indicators (both applied identically
    to every extended disease). Called once per disease in
    EXTENDS_DISEASE_NAMES -- unlike the Pulmonary importer, Acute Renal
    Failure and Chronic Renal Failure receive DIFFERENT new_concepts sets,
    reflecting that PR #40 built them as genuinely distinct diseases."""
    ...
    for c in manifest.get("new_concepts", []):
        if disease.disease_name not in c.get("diseases", []):
            continue
        domain = c["domain"]
        name = c["name"]
        normalized = name.strip().lower()
    ...
    # --- new applicability edges (only those declared for this disease) ---
    for a in manifest.get("new_applicability", []):
        if a.get("disease") != disease.disease_name:
            continue
        ...
        if concept_key not in concept_by_key:
            raise RuntimeError(
                f"Renal Clinical Evidence Blueprint v1 applicability mapping references a concept that "
                f"was not created on {disease.disease_name!r}: concept={concept_name!r} domain={concept_domain!r}. "
        ...
        db.add(
            OntologyConceptVariantApplicability(
                ...
                variant_id=variant_row.id,
                applicability_type=applicability_type,
                description="Imported from the approved Renal Clinical Evidence Blueprint v1.",
    ...
    # --- functional assessment framework linkage (OntologyRelationship, applied to every disease) ---
    for link in manifest.get("functional_assessment_linkage", []):
        renal_concept = _resolve_concept(db, disease.id, link["chf_concept_domain"], link["chf_concept_name"])
    ...
    # --- recertification trend indicators (PROGNOSTIC_INDICATOR, applied to every disease) ---
    existing_trend = {
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Renal Clinical Evidence Blueprint v1 failed structural/vocabulary validation: {errors}")
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    diseases (using each disease's own applicable subset for concepts and
    applicability, since this manifest does not force symmetry) -- plus
    provenance coverage and the second-run new-row count. Never a clinical
    judgment -- purely a mechanical comparison."""
    faf_disease = _resolve_disease(db, FAF_DISEASE_NAME)
    diseases = [_resolve_disease(db, name) for name in EXTENDS_DISEASE_NAMES]
    ...
    for disease in diseases:
        expected_concept_keys = {
            (c["domain"], c["name"].strip().lower())
            for c in all_new_concepts if disease.disease_name in c.get("diseases", [])
        }
        total_expected_concepts += len(expected_concept_keys)
        ...
        expected_applicability_for_disease = [a for a in all_applicability if a.get("disease") == disease.disease_name]
        total_expected_applicability += len(expected_applicability_for_disease)
        for a in expected_applicability_for_disease:
    ...
    return {
        ...
        "concepts": {
            "expected": total_expected_concepts,
            "stored": total_concepts_stored,
            "missing": total_missing_concepts,
            "pre_existing_pr40_concepts_in_shared_domains": pre_existing_concepts_all,
        },
        "applicability": {
            "expected": total_expected_applicability,
