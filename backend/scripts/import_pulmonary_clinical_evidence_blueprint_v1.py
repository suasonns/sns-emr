# scripts/import_pulmonary_clinical_evidence_blueprint_v1.py
"""
Pulmonary Clinical Evidence Blueprint v1 -- Extension-Only Importer.

Reads backend/manifests/pulmonary_clinical_evidence_blueprint_v1.json (the
sole authoritative source for this import) and EXTENDS the existing
"Chronic Obstructive Pulmonary Disease" AND "End Stage Pulmonary Disease"
diseases created by the Pulmonary Production Source Manifest (PR #39,
merged). Unlike the CHF and Dementia Clinical Evidence Blueprints (which
each extend a single disease), this manifest intentionally extends BOTH
mirrored pulmonary diseases identically -- PR #39 built them as two
parallel disease identities with an identical variant/concept structure,
...
- NEVER creates a new OntologyBodySystem, OntologyDiseaseFamily, or
  OntologyDisease row. If either "Chronic Obstructive Pulmonary Disease",
  "End Stage Pulmonary Disease", or "Functional Assessment Framework"
  (PR #49, merged) does not already exist, the import aborts before any
  writes.
- NEVER creates a new OntologyDiseaseVariant row.
- NEVER duplicates PR #39's or PR #49's PPS/KPS definitions, and NEVER
  duplicates a symptom/finding/nutritional concept already present in
  PR #39 (see the manifest's already_present_verified.mapping). Instead
  it adds two OntologyRelationship edges per disease linking each
  disease's existing PPS Less Than 70 Percent / KPS Less Than 70 Percent
  concepts to the Functional Assessment Framework's existing PPS/KPS
  OntologyDiseaseFinding rows.
- Adds two genuinely missing atomic Pulmonary concepts (Activity
  Intolerance, Severe Functional Limitation), applied identically to
  both diseases, and 9 Pulmonary recertification-trend
...
Run with: .\\.venv\\Scripts\\python.exe scripts\\import_pulmonary_clinical_evidence_blueprint_v1.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "pulmonary_clinical_evidence_blueprint_v1.json"
)
DEFAULT_ACCEPTANCE_PATH = (
    Path(__file__).resolve().parent.parent / "artifacts" / "pulmonary_clinical_evidence_blueprint_acceptance_v1.json"
)

EXTENDS_DISEASE_NAMES = ["Chronic Obstructive Pulmonary Disease", "End Stage Pulmonary Disease"]
...
ALLOWED_CONCEPT_DOMAINS = {"SYMPTOM", "FINDING", "NUTRITIONAL_IMPACT", "COMPLICATION", "FUNCTIONAL_IMPACT"}
...
# already-existing Pulmonary/FAF concepts referenced by name (never to create).
...
def validate_manifest(manifest: dict) -> List[str]:
    ...
    if scope.get("extends_diseases") != EXTENDS_DISEASE_NAMES:
        errors.append(
            ...
            "['Chronic Obstructive Pulmonary Disease', 'End Stage Pulmonary Disease']"
        )
    ...
    for a in manifest.get("new_applicability", []):
        ...
        if not a.get("variant_dimension"):
            errors.append("new_applicability entry missing variant_dimension (required -- pulmonary variant "
...
def _resolve_disease(db: Session, name: str) -> OntologyDisease:
    ...
    if disease is None:
        raise RuntimeError(
            f"Pulmonary Clinical Evidence Blueprint v1 requires the disease {name!r} to already exist. "
...
def _resolve_variant(db: Session, disease_id, variant_name: str, variant_dimension: str | None = None) -> OntologyDiseaseVariant:
    ...
    is provided. A dimension is REQUIRED whenever the pulmonary disease has
    more than one variant sharing the same name across dimensions (e.g.
    'End Stage Pulmonary Disease' exists as both a SEVERITY_CLASS and a
    DISEASE_PHASE variant) -- omitting it in that case raises rather than
    ...
    if len(matches) == 0:
        raise RuntimeError(
            f"Pulmonary Clinical Evidence Blueprint v1 references variant {variant_name!r} "
            f"(dimension={variant_dimension!r}) which was not found on the existing pulmonary disease. "
            "Aborting without any writes."
        )
    if len(matches) > 1:
        raise RuntimeError(
            f"Pulmonary Clinical Evidence Blueprint v1 references variant {variant_name!r} which is "
...
def _resolve_concept(db: Session, disease_id, domain: str, name: str):
    ...
    if row is None:
        raise RuntimeError(
            f"Pulmonary Clinical Evidence Blueprint v1 references concept {name!r} (domain={domain}) which "
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, content_source_type: str,
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="pulmonary_clinical_evidence_blueprint_v1",
            ...
            notes=_provenance_text(
                ...
                f"Imported from the approved Pulmonary Clinical Evidence Blueprint v1. {description}",
            ),
        )
    )
    return True
...
def _run_for_disease(db: Session, manifest: dict, disease: OntologyDisease, faf_disease: OntologyDisease) -> dict:
    ...
    single already-existing pulmonary disease. Called once per disease in
    ...
    # --- new applicability edges (new concept -> existing pulmonary variant) ---
    for a in manifest.get("new_applicability", []):
        ...
        if concept_key not in concept_by_key:
            raise RuntimeError(
                f"Pulmonary Clinical Evidence Blueprint v1 applicability mapping references a concept that "
        ...
        db.add(
            OntologyConceptVariantApplicability(
                ...
                variant_id=variant_row.id,
                applicability_type=applicability_type,
                description="Imported from the approved Pulmonary Clinical Evidence Blueprint v1.",
    ...
    for link in manifest.get("functional_assessment_linkage", []):
        pulm_concept = _resolve_concept(db, disease.id, link["chf_concept_domain"], link["chf_concept_name"])
        ...
        existing_rel = (
            ...
            .filter_by(
                ...
                source_concept_id=pulm_concept.id,
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Pulmonary Clinical Evidence Blueprint v1 failed structural/vocabulary validation: {errors}")
...
def build_acceptance_report(db: Session, manifest: dict, second_run_new_rows: int) -> dict:
    ...
    return {
        ...
        "concepts": {
            ...
            "pre_existing_pr39_concepts_in_shared_domains": pre_existing_concepts_all,
        },
        "applicability": {
