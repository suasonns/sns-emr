# scripts/import_liver_production_source_manifest.py
"""
Liver Production Source Manifest v1 -- Verbatim Importer.

Reads backend/manifests/liver_production_source_manifest_v1.json
(the sole authoritative source for this import -- never inferred,
reconstructed, or clinically re-derived) and creates the Hepatic
...
scripts/import_pulmonary_production_source_manifest.py (PR #39), and
...
Critical LCD rule: liver diagnosis alone is never sufficient to infer
...
never name-only matching, since e.g. "Child-Pugh Class C Cirrhosis" and
"End Stage Liver Disease" / "Chronic Liver Disease" style names recur
across both Tier 4 variant dimensions and Tier 5 concept domains.

Run with: .\\.venv\\Scripts\\python.exe scripts\\import_liver_production_source_manifest.py
"""
...
DEFAULT_MANIFEST_PATH = (
    Path(__file__).resolve().parent.parent / "manifests" / "liver_production_source_manifest_v1.json"
)
...
def _resolve_or_create_diseases(db: Session, manifest: dict) -> Dict[str, OntologyDisease]:
    ...
    Hepatic System body system, the declared family, and the disease
    ...
    if len(family_names) != 1:
        raise RuntimeError(
            f"Liver Production Manifest v1 declares {len(family_names)} families; "
...
def _evidence_notes(concept_entry: dict) -> str:
    ...
    parts = [
        "Imported verbatim from the approved Liver Production Source Manifest v1.",
...
def _ensure_evidence_rule(db: Session, concept_type: str, concept_id, concept_entry: dict | None = None) -> bool:
    ...
    db.add(
        OntologyEvidenceRule(
            ...
            evidence_source="liver_production_source_manifest_v1",
...
def run(db: Session, manifest: dict | None = None) -> dict:
    ...
    if errors:
        raise RuntimeError(f"Liver Production Manifest v1 failed structural/vocabulary validation: {errors}")
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for v in disease_entry.get("variants", []):
            ...
            variant = OntologyDiseaseVariant(
                ...
                source_reference="liver_production_source_manifest_v1",
            )
            db.add(variant)
            db.flush()
    ...
    for disease_entry in manifest["diseases"]:
        ...
        for a in disease_entry.get("applicability", []):
            ...
            if variant_key_found is None or concept_key_found is None:
                raise RuntimeError(
                    f"Liver Production Manifest v1 applicability mapping references a variant/concept "
            ...
            edge = OntologyConceptVariantApplicability(
                ...
                variant_id=variant_key_found.id,
                applicability_type=applicability_type,
                description="Imported verbatim from the approved Liver Production Source Manifest v1.",
                evidence_requirement=(
