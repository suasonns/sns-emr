...
Note: originally authored against down_revision 8d3911bbf250, which was the
alembic head on the source branch at the time. That revision no longer
exists in the repository's migration history (superseded/renamed by later
merges into origin/main). Repointed to origin/main's current actual head,
c8e6e7eef2d6 ("add facility collection alert lifecycle columns"), so this
isolated cherry-pick applies cleanly on top of current origin/main with a
single linear alembic head. No other migration file was changed.
"""
...
down_revision: Union[str, Sequence[str], None] = "c8e6e7eef2d6"
