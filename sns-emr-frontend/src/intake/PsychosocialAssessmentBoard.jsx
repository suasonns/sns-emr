...
import { listMswIcaAssessmentsByPatient } from "../api/icaAssessments";
...
export default function PsychosocialAssessmentBoard({ patientId = "", selectedAssessmentId: externallySelectedAssessmentId = null }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    listMswIcaAssessmentsByPatient(patientId)
      .then((result) => {
        if (!mounted) return;
      ...
      .catch((loadError) => {
        if (!mounted) return;
        setRecords([]);
        setError(loadError?.message || "Unable to load psychosocial assessment history.");
      })
      .finally(() => {
    ...
    <div style={styles.shell}>
      <div style={styles.card}>
        <div>
          ...
          <h2 style={styles.title}>{initialComplete ? "Comprehensive Psychosocial Assessment" : "MSW Initial Comprehensive Assessment"}</h2>
          <div style={styles.description}>
            {initialComplete
              ? "Real psychosocial assessment records for this patient are listed below and can be reopened directly."
              : "No locked psychosocial assessment is on file yet. When one exists, it will appear below automatically."}
          </div>
        </div>
      ...
      <div style={styles.historyCard}>
        {loading ? (
          <div style={{ fontSize: 13, color: "#94A3B8" }}>Loading psychosocial history…</div>
        ) : error ? (
          <div style={{ fontSize: 13, color: "#FCA5A5" }}>{error}</div>
        ) : records.length === 0 ? (
          <div style={{ fontSize: 13, color: "#94A3B8" }}>No MSW ICA records are on file for this patient yet.</div>
        ) : (
                ...
                {records.map((record) => (
                  <tr key={record.assessmentId}>
                    <td style={styles.historyTd}>
                      <div style={{ fontWeight: 700 }}>{record.assessmentType || "MSW ICA"}</div>
      ...
      <MSWICA patientId={patientId} assessmentId={selectedRecord?.assessmentId} mode="ica" />
    </div>
  );
}
