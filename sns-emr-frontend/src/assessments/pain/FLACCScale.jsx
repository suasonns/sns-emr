...
const getInterpretation = (total) => {
  if (total === 0) return { label: 'Relaxed / Comfortable', color: COLORS.green, bg: COLORS.greenBg };
  if (total <= 3) return { label: 'Mild Discomfort', color: '#84cc16', bg: '#84cc1615' };
...
const FLACCScale = () => {
  ...
  return (
    <div style={{ backgroundColor: COLORS.card, borderRadius: 8, padding: 24, borderLeft: `4px solid ${COLORS.teal}` }}>
      <div style={{ color: COLORS.white, fontSize: 15, fontWeight: 700, marginBottom: 4 }}>FLACC — Behavioral Pain Assessment Scale</div>
      <div style={{ color: COLORS.label, fontSize: 12, marginBottom: 12 }}>For infants, young children, and non-verbal critical care patients.</div>
